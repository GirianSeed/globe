...
// 0.01           Jul,29,2002     Akira Sonoda Modified type of total/maxsector
// 0.02           Jun,01,2003     Akira Sonoda It coped with CodeWarrior.
...
#ifdef __MWERKS__
#define error(...)	printf("%s: ", bname), printf(__VA_ARGS__)
#else
...
static int ShowPartition(const char *devname) {
    int i, r, n, fd;
    char size[16];
    char type[16];
...
static void DoCat(const CmdType *cmd, HelpLevel hl, c_char *p) {
    ...
    if (hl) {
	PrShort(cmd, "<[dev:]file>", "display contents of file");
	return;
    }
...
static void DoCd(const CmdType *cmd, HelpLevel hl, c_char *p) {
    ...
    if (hl) {
	PrShort(cmd, "[[dev:]directory]","change current directory");
	return;
    }
...
static void DoCp(const CmdType *cmd, HelpLevel hl, c_char *p) {
    ...
    if (hl) {
	PrShort(cmd, "<[dev:]file> <[dev:]file>", "copy file");
	return;
    }
...
//  if (!*p) return DoCp(cmd, HELP_LONG, NULL);
    ...
    if ((wfd = sceOpen(buf1, SCE_RDWR|SCE_CREAT|SCE_TRUNC, cmode)) < 0) {
	printf("cannot open file:%s, %d\n", buf1, wfd);
	sceClose(rfd);
	return;
    }
...
static void DoExec(const CmdType *cmd, HelpLevel hl, c_char *p) {
    if (hl) {
	PrShort(cmd, "<[dev:]file>", "execute file");
	return;
    }
...
static void DoFdisk(const CmdType *cmd, HelpLevel hl, c_char *p) {
    ...
    if (hl) {
	PrShort(cmd, "[dev:]", "partition manipulator");
	return;
    }
...
static void DoFormat(const CmdType *cmd, HelpLevel hl, c_char *p) {
    ...
    if (hl) {
	PrShort(cmd, "<dev:> [zone size]", "format device");
	return;
    }
...
static void DoMkdir(const CmdType *cmd, HelpLevel hl, c_char *p) {
    ...
    if (hl) {
	PrShort(cmd, "<directory>", "make directory");
	return;
    }
...
static void DoMount(const CmdType *cmd, HelpLevel hl, c_char *p) {
    ...
    if (hl) {
	PrShort(cmd, "<block device> <fs>", "mount filesystem");
	return;
    }
...
static void DoMv(const CmdType *cmd, HelpLevel hl, c_char *p) {
    ...
    if (hl) {
	PrShort(cmd, "<[dev:]file> <[dev:]file>", "move file");
	return;
    }
...
static void DoPlist(const CmdType *cmd, HelpLevel hl, c_char *p) {
    ...
    if (hl) {
	PrShort(cmd, "[dev:]", "list partitions");
	return;
    }

    if (*p) r = ShowPartition(p);
    else    r = ShowPartition("hdd0:");
    if (r < 0) error("cannot list partitions, %d\n", r);
}
...
static void DoRmdir(const CmdType *cmd, HelpLevel hl, c_char *p) {
    ...
    if (hl) {
	PrShort(cmd, "<directory>", "remove directory");
	return;
    }
...
static void DoUmount(const CmdType *cmd, HelpLevel hl, c_char *p) {
    ...
    if (hl) {
	PrShort(cmd, "<fs>", "unmount filesystem");
	return;
    }
...
static void DoChdev(const CmdType *cmd, HelpLevel hl, c_char *p) {
    if (hl) {
	PrShort(cmd, "", "change current device");
	return;
    }
}
...
static void DoHelp(const CmdType *cmd, HelpLevel hl, c_char *p) {
    ...
    if (hl) {
	PrShort(cmd, "[command]", "print command usage");
	return;
    }
...
//	    commands[i].exec(&commands[i], HELP_LONG, NULL);
