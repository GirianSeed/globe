...
/* IOPフレームカウント取得関数 (デバッグ用) */
extern int scePadGetFrameCount( int port, int slot );

/* 描画環境定義 */
#define SCREEN_WIDTH	640			/* 画面解像度(x)          */
#define SCREEN_HEIGHT	448			/* 画面解像度(y)          */
#define OFFX	(((4096-SCREEN_WIDTH)/2)<<4)	/* 画面左上始点座標(x)    */
#define OFFY	(((4096-SCREEN_HEIGHT)/2)<<4)	/* 画面左上始点座標(y)    */
...
static int attach_controller(TSLOTDATA* pSlot)
{
	...
	switch(phase)
	{
		...
		case 77:
		/* scePadEnterPressMode()の実行結果チェック */
		if (scePadGetReqState(port, slot)==scePadReqStateFaild)
		{
			phase--;
		}
