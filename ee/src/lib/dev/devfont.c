...
static void sceDevFontRefDirectImage(sceGifPacket * pPacket, u_long gsxyz, u_int dy, u_int rgba,
									 u_long image, u_int ctxt);
static u_int sceDevFontRefStrN(sceGifPacket * pPacket, sceDevFontInfo const *pFont, u_int x,
							   u_int y, u_short const *str, u_int count);
...
void sceDevConsDrawS(int const cd)
{
	...
	for (y = 0; y < h; y++) {
		...
		while (x < w) {
			...
			sceGsSyncPath(0, 0);
			sceDmaSend(devGif,
					   (u_long128 *) (((u_int) packet[side].pBase & 0x3fff) | 0x80000000));
...
u_int sceDevConsPrintf(int const cd, char const *const str, ...)
{
...

	{
		...
		va_start(list, str);
		sceVsnprintf(work, 1024, str, list);
		va_end(list);
	}
...
void sceDevConsMove(int const cd, int const dx, int const dy, int const sx, int const sy,
					u_int const w, u_int const h)
{
	sceDevCons *const pCons = (sceDevCons *) cd;
	u_int lx;
	u_int ly;
...
static void sceDevFontRefDirectImage(sceGifPacket * const pPacket, u_long gsxyz, u_int dy,
									 u_int const rgba, u_long image, u_int const ctxt)
{
	if (0L == image) {
		return;
	}
...
static u_int sceDevFontRefStrN(sceGifPacket * const pPacket, sceDevFontInfo const *const pFont,
							   u_int const x, u_int const y, u_short const *str, u_int count)
{
	u_int result = 0;
	...
	while (count--) {
...

		{
			...
			sceDevFontRefDirectImage(pPacket, SCE_GS_SET_XYZ(fx, fy, 0xffffffff), dy, rgba, image,
									 pFont->ctxt);
		}
...
{
	if ((x < 0) || (x >= pCons->width)) {
		return;
	}
	if ((y < 0) || (y >= pCons->height)) {
		return;
	}
...
static void sceDevConsPiece(sceDevCons * const pCons, int const x, int const y, u_char c,
							u_char const a)
{
	if ((x < 0) || (x >= pCons->width)) {
		return;
...
static void chaGifPkOpenGifTag2(sceGifPacket * const pPacket, u_long const tagLow,
								u_long const tagHigh)
{
...
u_long128 *sceDevFont(u_long128 * p, int x, int y, int z, char *s, int ctxt)
{
	...
	while ('\0' != (c = *s++)) {
...

		{
			...
			sceDevFontRefDirectImage(&packet, SCE_GS_SET_XYZ(fx, fy, 0xffffffff), dy, 0x80ffffff,
									 image, ctxt);
		}
