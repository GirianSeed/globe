...
Sample program that uses libhttp to connect to an HTTP server, then reconnects to a redirection destination when redirection is specified.

<Description>
This is a sample program that uses the redirection function of libhttp to reconnect to a redirection destination when redirection is specified from the HTTP server.
...
<Run Method>
	...
	$ dsedb -r run http_redirect.elf <URL>	: Execute

An HTTP connection is made to the specified URL, then a reconnection is made to the redirection destination when redirection is specified from the HTTP server.
...
3. If a code indicating redirection was specified in the HTTP response, make the HTTP connection again to the redirection destination and obtain an HTTP response.
