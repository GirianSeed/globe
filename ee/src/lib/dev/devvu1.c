...
static u_int sceVuCheckBusy(){
  ...
  if(0x00000100 == (reg & 0x00000f00)){ return 1; }	/*	vu1 busy ?	*/
  return 0;
}
...
static void sceVu0MemWriteQ(u_short vuaddr, qword value){
  ...
  __asm__ __volatile__("cfc2 %0, $vi01" : "=r"(ibak));			/*	VU0-VI01を待避				*/
  __asm__ __volatile__("qmfc2 %0, $vf01" : "=r"(fbak));			/*	VU0-VF01をfbakへ待避		*/

  __asm__ __volatile__("qmtc2 %0, $vf01" :: "r"(reg));			/*	VU0-VF01へvalueをロード		*/

  __asm__ __volatile__("ctc2 %0, $vi01" ::"r"(vuaddr));			/*	VU0-VI01にアドレスをセット	*/
  __asm__ __volatile__("vsqi.xyzw $vf01, ($vi01++)");			/*	VU0-MEM[vuaddr]へデータ設定	*/

  __asm__ __volatile__("ctc2 %0, $vi01" ::"r"(ibak));			/*	VU0-VI01を復帰				*/
  __asm__ __volatile__("qmtc2 %0, $vf01" :: "r"(fbak));			/*	VU0-VF01を復帰				*/
}
...
static qword sceVu0MemReadQ(u_short vuaddr){
  ...
  __asm__ __volatile__("ctc2 %0, $vi01" ::"r"(vuaddr));			/*	VU0-VI01にアドレスをセット	*/
  __asm__ __volatile__("vlqi.xyzw $vf01, ($vi01++)");			/*	VU0-MEM[vuaddr]のデータ取得	*/

  __asm__ __volatile__("qmfc2 %0, $vf01" :"=r"(reg));			/*	VU0-VF01からデータ取得		*/
...
void sceDevVu1Exec(u_short addr){
  ...
  __asm__ __volatile__("ctc2 %0, $vi31" ::"r"(reg));
}
