...
<Files>
	...
	term.txt	: Network configuration application item 
	...
	dev_more.c	: Detailed setting process of connection device
	ifc.c		: Setting process of connection provider
	ifc_more.c	: Detailed setting process of connection 
			  provider
...
setapp.h also contains a description of the USB autoload configuration file path. This configuration file contains settings related to the USB-Ethernet interface driver an986.irx, which SCEI currently provides for development purposes. When a USB Modem is used, a USB Modem driver and a DIAL_CNF PATH definition must be added to this setting file.  
...
<Limitations and precautions>
	...
	- Although setapp has been created as a reference for operating flow, and so on, in accordance with the network configuration application specifications, processes related to input interface and memory cards are easily implemented. 

<Notes on Creating Applications>

	- In a title, the items, item names, and terms that are used in this sample program should be replaced with the corresponding Japanese items, item names, and terms.
