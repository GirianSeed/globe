...
Sample program that uses libhttp to connect to an HTTP server, then performs a chunk-format encoding receive.

<Description>
This sample program uses libhttp to connect to an HTTP server, then performs a chunk-format encoding receive.
...
<Run Method>
	...
	$ dsedb -r run http_chunk.elf <URL>	: Execute

	An HTTP connection is made to the specified URL and the result is displayed.
...
2. Use sceHTTPSetOption() to set the callback function for chunk receives.
