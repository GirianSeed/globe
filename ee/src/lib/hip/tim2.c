 ...
 *                        All Rights Reserved.
 */
/*	$Id: tim2.c,v 1.14.2.1 2004/07/07 05:03:54 xkazama Exp $	*/
#include <eekernel.h>
#include <stdlib.h>
#include <eeregs.h>
#include <libgraph.h>
...
static int      _curpnum;               /* current picture number for save dma buffer */
...
static sceHiErr MakeTim2TexturePacket(int psm, u_int tbp, int tbw, int w, int h, int x, int y, u_long128 *pImage)
{
    ...
    if (err != SCE_HIG_NO_ERR)
	return err;
    err = sceHiDMAGet_BufferPtr(&(_curtf->pbufaddr[_curpnum * 8 + _curimg]));
    if (err != SCE_HIG_NO_ERR)
	return err;
...
{ 
    ...
    /* allocate the area that has buffer-addresses */
    tim2->pbufaddr = (u_int **) sceHiMemAlloc(sizeof(u_int*) * 8 * dataH->num);
...
sceHiErr sceHiPlugTim2(sceHiPlug *plug, int process)
{
	...
	if (tim2_frame == NULL)
	    return _hip_tim2_err(_PLGBLK_STACK_BROKEN);
        if (tim2_frame->pbufaddr != NULL)
            sceHiMemFree((u_int *)Paddr(tim2_frame->pbufaddr));
...
sceHiErr sceHiPlugTim2SetPicture(sceHiPlug *plug, int n, int num)
{
...
#endif
	    err = sceHiDMASet_BufferPtr(tf->pbufaddr[n * 8 + 0]);
	    if (err != SCE_HIG_NO_ERR)
		return err;
...
#endif
		err = sceHiDMASet_BufferPtr(tf->pbufaddr[n * 8 + i]);
		if (err != SCE_HIG_NO_ERR)
		    return err;
		err = sceHiDMAMake_LoadGS((u_int *)pImage, _calc_image_size(psm, w, h));
		if (err != SCE_HIG_NO_ERR)
	...
	if (ph->ClutType != TIM2_NONE) {
	    ...
	    switch ((ph->ClutType << 8) | ph->ImageType) {
	      ...
	      case (( TIM2_RGB32         << 8) | TIM2_IDTEX8): /* 256 CSM1 32bit */
		{
		    err = sceHiDMASet_BufferPtr(tf->pbufaddr[n * 8 + 7]);
		    if (err != SCE_HIG_NO_ERR)
			return err;
