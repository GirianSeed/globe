...
static void rcv_cb(void *p, char *cdata, int clen, void *uopt){
	scePrintf("<<Receive data chunk (%x, %d)>>\n", (unsigned)cdata, clen);
	free(cdata);
}
...
int http_chunk_init(int argc, char **argv){
	...
	if(argc < 2){
		scePrintf("usage: http_chunk.elf url\n");
		return -1;
	}
...
int http_chunk(void){
	...
	if (sceHTTPSetOption (client, sceHTTPO_HTTPRevision, 1) < 0){
	...
	if (sceHTTPSetOption(client, sceHTTPO_ReceiveChunkCB, rcv_cb, NULL) < 0){
		scePrintf("sceHTTPSetOption failed\n");
		return -1;
	}
