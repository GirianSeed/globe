...
void clearGsMem(int r, int g, int b, int disp_width, int disp_height)
{
  ...
  // setting GIF tag for db.draw0 structure
  SCE_GIF_CLEAR_TAG(&db.giftag0);
    __asm__ __volatile__ ("sync.l"); // wait until giftag is filled with 0
  db.giftag0.NLOOP = 8;
  db.giftag0.EOP = 1;
  db.giftag0.NREG = 1;
...
void setImageTag(u_int *tags, void *image, int index, int image_w, int image_h)
{
  ...
  if (index == 0) {  // set image data to packet
	...
	sceGifPkRef(&packet, DmaAddr(image), 16*16*4/16, 0, 0, 0);
...
int vblankHandler(int val)
{
  ...
  if (isCountVblank) {
	...
	return 0;
      ...
      sceGsSetHalfOffset((oddeven&1)?(sceGsDrawEnv1*)UncAddr(&db.draw1)
			 :(sceGsDrawEnv1*)UncAddr(&db.draw0),
