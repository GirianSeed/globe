 ...
 *  --------------------------------------------------------------------
 *      1.00            June,9,1999     akiyuki     first version
 */
...
#define PI (3.141592f)
#define SCE_PADreset	(SCE_PADstart|SCE_PADselect|SCE_PADL1|SCE_PADL2|SCE_PADR1|SCE_PADR2)
...
#define	THREADS		(58)	// thread最大数(64-main-sync-point-dummy-kernel-pad)
#define	NL		(210.f)	// object間の距離
#define	NZ		(2000.f)// objectのZ位置
#define	CAMZ		(0.f)	// cameraのZ位置
#define	NX		(8)	// object表示数(x)
#define	NY		(8)	// object表示数(y)
#define	STACKSIZE	(256*16)// stack size
#define	PKTSIZE		(1024)	// qword (2+THREADS*(26/2)+1 以上)
...
struct RotateData {
	...
	int		id;	// thread id
	int		no;	// thread no
	unsigned int	autorotate : 1;// 自動回転フラグ
	unsigned int	command : 1;// key check flag
};
...
{
	if ( rd->pad->button & SCE_PADselect ) {			// init key
		if ( rd->pad->button & SCE_PADR1 ) {
			rd->trans[0] = rd->trans[1] = 0.f;
		}
		if ( rd->pad->button & SCE_PADR2 ) {
			rd->trans[0] = NL*(rd->no%NX)-NL*(NX-1)/2.f;
			rd->trans[1] = NL*(rd->no/NX)-NL*(NY-1)/2.f;
		}
		if ( rd->pad->button & SCE_PADLright || rd->pad->button & SCE_PADLleft ) {
			rd->trans[0] = NL*(rd->no%NX)-NL*(NX-1)/2.f;
		}
		if ( rd->pad->button & SCE_PADLdown || rd->pad->button & SCE_PADLup ) {
			rd->trans[1] = NL*(rd->no/NX)-NL*(NY-1)/2.f;
		}
		if ( rd->pad->button & SCE_PADL1 || rd->pad->button & SCE_PADL2 ) {
			rd->trans[2] = NZ;
		}
		if ( rd->pad->button & SCE_PADRleft ) {
		...
		if ( rd->pad->button & SCE_PADLright ) rd->trans[0] += 20.f;
		if ( rd->pad->button & SCE_PADLleft  ) rd->trans[0] -= 20.f;
		if ( rd->pad->button & SCE_PADLdown  ) rd->trans[1] += 20.f;
		if ( rd->pad->button & SCE_PADLup    ) rd->trans[1] -= 20.f;
		if ( rd->pad->button & SCE_PADL2     ) rd->trans[2] += 20.f;
		if ( rd->pad->button & SCE_PADL1     ) rd->trans[2] -= 20.f;
	}
// 一定速度で回転
	if ( rd->autorotate ) {
...
void ThreadStart( void *thread, struct RotateData *rd )
{
...
/*
	...
	3. point threadを開始
*/
int StartObjects( struct RotateData *rd, int sz_rd, int num, scePadData *pad,
	sceVif1Packet *packet, sceVu0FMATRIX *WorldView, sceVu0FMATRIX *ViewScreen,
	sceVu0FMATRIX *NormalLight, sceVu0FMATRIX *LightColor )
{
	int i, id;
	static struct ThreadParam pt;
	static char stack[192*16] __attribute__ ((aligned(16)));
	// set objects data
	rd[0].autorotate = 1;
	rd[0].pad = pad;
	rd[0].packet = packet;
	...
	// start objects
	for ( i = 0; i < num; i++ ) {
		ThreadStart( (void*)CubeObject, &rd[i] );
	}
	// start PointThread
...
void SyncThread( void *data )
{
	...
	fd = sceOpen( "pad:0", SCE_RDONLY );
	InitMatrix( WorldView, ViewScreen, NormalLight, LightColor );
	StartObjects( rd, bulkof(rd), num, &pad, &packet, &WorldView,
		&ViewScreen, &NormalLight, &LightColor );
	sceGsResetPath();
	sceDmaReset( 1 );
	...
	for ( ; ; ) {
		...
		sceRead( fd, &pad, sizeof(pad) );// パッド読み込み
		if ( ( pad.button & SCE_PADreset ) == SCE_PADreset ) {
			Exit( 0 );		// 終了キー
		}
		if ( pad.button & SCE_PADRright && num < THREADS ) {
			ThreadStart( (void*)CubeObject, &rd[num++] );
		}
		if ( pad.button & SCE_PADRdown && 0 < num ) {
			ThreadStop( rd[--num].id );
		}
