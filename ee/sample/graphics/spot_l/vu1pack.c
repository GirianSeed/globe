...
extern   sceVu0FVECTOR plight2;
extern   sceVu0FVECTOR light0;
extern   sceVu0FVECTOR light1;
extern   sceVu0FVECTOR light2;
extern   sceVu0FVECTOR light0_dir;
extern   sceVu0FVECTOR light1_dir;
extern   sceVu0FVECTOR light2_dir;
			      ...
			      sceVu0FVECTOR obj_trans)
{
  ...
  head->current->dmatag.ui32[0] = 12 | ((0x5)<<28);// DMAtag [call 0x5] 
  ...
  sceVu0ApplyMatrix(tmp, work_mat, light0_dir);
  sceVu0CopyVector(head->current->Light_vector[0], tmp);// rotation only
  sceVu0ApplyMatrix(tmp, work_mat, light1_dir);
  sceVu0CopyVector(head->current->Light_vector[1], tmp);// rotation only
  sceVu0ApplyMatrix(tmp, work_mat, light2_dir);
  sceVu0CopyVector(head->current->Light_vector[2], tmp);// rotation only
  ...
  sceVu0ApplyMatrix(tmp, work_mat, plight2);
  sceVu0CopyVector(head->current->Light_point[2],tmp);

  head->current->viftag.ul128 = 0x0;
  head->current->viftag.ui32[3] 
    = SCE_VIF1_SET_UNPACK(25, 6, 0x0c, 0);// free area 25-29
...
Vu1EnvPacket* CloseVu1EnvPacket(Vu1DmaPacket *dma)
{
    ...
    dma->envhead.Vifend.ui32[1] = SCE_VIF1_SET_BASE(41, 0);// BASE 21 - 491
    dma->envhead.Vifend.ui32[2] = SCE_VIF1_SET_OFFSET(480, 0);// OFFSET 491-961
