...
// ワークバッファ
inhibitChar_t EnableChar;                                // 入力許可文字       禁止文字へ変換前のワーク
...
int KeyBoardSet_Setup( KEYBOARD_SET* kbSet, tSkb_Ctl* skbCtl, int region  )
{
	...
	// モードバー設定
	{
		...
		modeBarGRAPH = MODEBAR_SKBTYPE_GRAPH_AE; // 海外用
			...
			if( region == REGION_J )  modeBarGRAPH = SKBTYPE_MODEBAR_J;
			else                      modeBarGRAPH = SKBTYPE_MODEBAR_AE;
			#endif
		#endif
...
int SKB_Init()
{
	...
	//--------------------------------------------------
	{
		...
		Fep_Init(&g_import   , g_import.ucUseAtok, atokPrio, SkbArg.AtokPath.sysDic, SkbArg.AtokPath.usrDic, SkbArg.AtokPath.system, SkbArg.AtokPath.config );
	}
	//---------------------------------------------------------------------------
...
// ソフトウェアキーボード  オープン
//
void SKB_Open( KEYBOARD_SET* kbSet, int editType, u_int* text )
{
	int n;
	int atype;
...
void SKB_SetInhibit_Disable( u_int (*ucs4)[10], int count )
{
	...
	//禁止文字情報を禁止情報バッファにコピー。
	SKB_inhibitCopy( &inhibitWork, &inhibitChar, INHIBIT_CHAR_MAX );
