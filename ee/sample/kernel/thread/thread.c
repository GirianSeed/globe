...
// THREADS の最大値は 256-main-sync-point-dummy-InitThread
#define	THREADS		(251)			// thread最大数
#define	SYNC_PRIO	(10)			// SyncThread priority
#define	ROTATE_PRIO	(20)			// rotate_thread priority
...
void SyncThread( void *data )
{
	...
	for ( ; ; ) {
		...
		// ※↓sceGsSwapDBuffの前に必要
		SyncDCache( &db, (char*)&db + sizeof(db) - 1 );
...
//		sceVif1PkDump( &packet ); Exit( -1 );	// packet dump
		FlushCache( 0 );
