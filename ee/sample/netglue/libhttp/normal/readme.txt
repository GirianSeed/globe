...
Sample program that uses libhttp to connect to an HTTP server.

<Description>
This is a sample program that uses libhttp to connect to an HTTP server.
libhttp enables blocking or non-blocking to be selected when making an HTTP connection. This sample program makes the connection using non-blocking.
...
<Run Method>
	...
	$ dsedb -r run http_test.elf command <URL> [post-data/file]: Execute
...
command specifies the HTTP method to be used. The following three commands can be specified.
		...
		-head		HEAD method
		-post		POST method
For the POST method, data to be POSTed can be specified with the post-data parameter. Also, for the GET method, you can specify the name of the file which will receive the response body.	
...
2. Use sceHTTPSetOption() to specify non-blocking.
