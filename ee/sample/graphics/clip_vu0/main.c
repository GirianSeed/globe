...
int main()
{
    ...
    *(u_long *)&db.clear1.rgbaq
    ...
    // set texture env
    texenv.size = sceGsSetDefTexEnv(&texenv.gs_tex, 0, 13340, IMAGE_SIZE / 64,
    ...
    *(u_long *) &alphaenv.gs_test =  SCE_GS_SET_TEST(0, 0, 0, 0, 0, 0, 3, 0x3);
	...
	sceRead(fd, &pad, sizeof(pad));
        ...
        if((pad.button & SCE_PADi & ~pre_button)){
             clip_switch = clip_switch^0x02; }
        if((pad.button & SCE_PADj & ~pre_button)){
             sf_switch = sf_switch^0x01;  }

        if((pad.button & SCE_PADselect & ~pre_button)){
             obj_switch = obj_switch^0x01;}
        if((pad.button & SCE_PADstart & ~pre_button)){
