...
static void sceVu0MemWriteQ(u_short vuaddr, qword value){
  ...
  asm("cfc2 %0, $vi01" : "=r"(ibak));			/*	VU0-VI01を待避				*/
  asm("qmfc2 %0, $vf01" : "=r"(fbak));			/*	VU0-VF01をfbakへ待避		*/

  asm("qmtc2 %0, $vf01" :: "r"(reg));			/*	VU0-VF01へvalueをロード		*/

  asm("ctc2 %0, $vi01" ::"r"(vuaddr));			/*	VU0-VI01にアドレスをセット	*/
  asm("vsqi.xyzw $vf01, ($vi01++)");			/*	VU0-MEM[vuaddr]へデータ設定	*/

  asm("ctc2 %0, $vi01" ::"r"(ibak));			/*	VU0-VI01を復帰				*/
  asm("qmtc2 %0, $vf01" :: "r"(fbak));			/*	VU0-VF01を復帰				*/
}
...
static qword sceVu0MemReadQ(u_short vuaddr){
  ...
  asm("ctc2 %0, $vi01" ::"r"(vuaddr));			/*	VU0-VI01にアドレスをセット	*/
  asm("vlqi.xyzw $vf01, ($vi01++)");			/*	VU0-MEM[vuaddr]のデータ取得	*/

  asm("qmfc2 %0, $vf01" :"=r"(reg));			/*	VU0-VF01からデータ取得		*/
