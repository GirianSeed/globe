...
#if USE_ERX
int main_overlay(const char *pszFileName)
#elif USE_OVERLAY
int main_balls(const char *pszFileName)
#else
...
#endif
{
    ...
    u_long giftag[2] __attribute__((aligned(16))) = {
			SCE_GIF_SET_TAG(0, 1, 0, 0, 1, 12),
			0x0000f43434343610L };	/* スプライト用GIFタグ */
    u_long giftagAD[2] __attribute__((aligned(16))) = {
    ...
    console = sceDevConsOpen(OFFX+(8<<4), OFFY+(8<<4), 30 , 1);
	...
	*T0_COUNT = 0;		/* Hcounter reset */
	...
	if(AutoStep()) {
		...
		printf("*** return from child balls ***\n");
	}
	if (paddata&SCE_PADselect) break;
#endif
	...
	sceDevConsPrintf(console, "%s", string_buf);
	sceGsSyncPath(0,0);
