...
libhttp を利用して、HTTP サーバに接続しチャンク形式
エンコーディング受信を行なうサンプル。

<サンプルの解説>
	本プログラムは libhttp を利用して HTTP サーバに
	接続し、チャンク形式エンコーディング受信を行なうサンプルです。
...
<ファイル>
	http_chunk.c        : メインプログラム

<起動方法>
	$ make                          : コンパイル

	$ dsedb -r run http_chunk.elf <URL> : 実行

	指定された URL に対して HTTP 接続を行ない、結果を表示します。
...
<処理概略>
	...
	2. sceHTTPSetOption() でチャンク受信時のコールバック関数を指定する
