...
Sample program that uses libhttp to connect to an HTTP server via a proxy server.

<Description>
This sample program uses the proxy function of libhttp to connect to an HTTP server via a proxy server.
...
<Run Method>
	...
	$ dsedb -r run http_proxy.elf <URL> <proxy_URL>	: Execute

The URL of the proxy server is set to proxy_URL, an HTTP connection is made to the specified URL, and the HTTP response is displayed.
...
2. Use sceHTTPSetOption() to set the URL of the proxy server.
3. Make an HTTP connection to the target URL via the proxy and obtain an HTTP response.
