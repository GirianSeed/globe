...
#define ATOK_CONFIG_PATH  ATOK_DEVICE"/atok/atok.ini"     // ATOKユーザー設定のパス
...
static u_int FepMode_NetCnfName[] =
{
...
KEYBOARD_SET KeyBoardSet_HiKanji = {
	...
	{ sizeof( FepMode_NetCnfName )/sizeof(u_int), FepMode_NetCnfName,  { FEP_BAR_X, FEP_BAR_Y }},
...
// Ucs -> Sjis 文字列への変換
static void UcsToSjis( u_int* ucs4, unsigned char *sjis, u_int bufMax )
{
	u_char utf8char[12] = "";
	...
	while( *ucs4 && bufMax >=0 ){
		...
		**ppUtf8 = 0;
		...
		sceCccUTF8toSJIS( sjis, bufMax, utf8char );
		len = strlen(sjis);
		bufMax-=len;
		sjis  +=len;
	}
...
int main()
{
	...
	*(u_long *)(void *)&g_db.clear1.rgbaq = SCE_GS_SET_RGBAQ(0x00, 0x20, 0x40, 0x00, 0);
	...
	/*---------------------------------------------------------------*/
	{
		...
		skbArg.AtokPath.config = ATOK_CONFIG_PATH;
	...
	// ソフトウェアキーボード初期化
	ret = SKB_Init();  // 返り値に、ＨＤＤアクセス中確保される、セマフォＩＤを返す。
	WaitPowerOff_SemaID = ret;  // 電源コントロール処理で、使用。
	...
	while(1){
		...
		if(! PowerOff_Start){
			if( SKB_GetStatus() ){
				...
				if( padOn & SCE_PADRright ){
					...
					SKB_SetInhibit_Disable( inhibitWorkBuf , 0 ); //入力、禁止文字なし。

					SjisToUcs4( TextBufSjis, TextBufUcs, 1024 );
					kbSet = &KeyBoardSet_All;
					SKB_Open( kbSet, 0, TextBufUcs ); // キーボードセット、編集動作モード、編集初期テキストを指定
				}
			}
		}
		if( SKB_GetStatus() )	pkSkb = SKB_MakeVif1CallPacket( 0, 0 );
		else                    pkSkb = (u_long128*)0;
		...
		if( fr ) {
			sceGsSetHalfOffset(&g_db.draw1, 2048, 2048, fr);
		} else {
			sceGsSetHalfOffset(&g_db.draw0, 2048, 2048, fr);
		}
		sceGsSwapDBuff( &g_db, fr );  // ダブルバッファ切り替え
		FlushCache(WRITEBACK_DCACHE); // Dキャッシュ内のデータをライトバック
