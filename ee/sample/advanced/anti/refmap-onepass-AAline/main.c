...
extern void create_cup(void);
extern void create_cup_line(void);
...
#define ANGLE_DELTA 1.00f
	...
	*(u_long*)&texenv.gs_tex.tex11 = SCE_GS_SET_TEX1(0, 0, 1, 1, 0, 0, 0);
	...
	//shaded triangles
	//配列PacketData1[]にカップのシェイディング表示用データを生成する。
	...
	//AA1 lines
	//配列PacketData3[]にカップと取っ手のアンチエイリアシング用のAA1ライン
	//データを生成する。
	...
	//配列PacketData4[]にカップの反射マッピング表示用データを生成する。
	TME_FLAG = 1;
	initialize_packet_buffer4();
	create_cup();
	close_packet_buffer4();

	//配列PacketData5[]に取っ手の反射マッピング表示用データを生成する。
	initialize_packet_buffer5();
	create_handle();
	close_packet_buffer5();

	//TME lines
	//配列PacketData6[]にカップと取っ手のアンチエイリアシング用のAA1ライン
	//データを生成する。(反射マッピング付き）
