 ...
 *       1.1.0     2003/04/02     ksh         USB to HDD Ethernet etc.
 */
...
int 	main( int argc, char* argv[] )
{
	...
	***************************************************************/
	...
	ret = load_set_conf_extra( &cd, g_net_buf, NET_DB, USR_CONF_NAME, sceLIBNETF_AUTO_UPIF );
	if ( ret < 0 ) {
	...
	ret = load_set_conf_extra( &cd, g_net_buf, NET_DB, USR_CONF_NAME, 0 );
	if ( ret < 0 ) {
		scePrintf( "load_set_conf_extra() failed.(%d)\n", ret );
		ERR_STOP;
	}
