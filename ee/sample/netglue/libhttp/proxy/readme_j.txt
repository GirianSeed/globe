...
libhttp を利用して、プロキシサーバ経由で
HTTP サーバへの接続を行なうサンプル。

<サンプルの解説>
	本プログラムは libhttp のプロキシ機能を利用して、
	プロキシサーバ経由で HTTP サーバへの接続を行なうサンプルです。
...
<ファイル>
	http_proxy.c        : メインプログラム

<起動方法>
	$ make                          : コンパイル

	$ dsedb -r run http_proxy.elf <URL> <proxy_URL> : 実行

	proxy_URL をプロキシサーバの URL として、指定された URL に対して
	HTTP 接続を行ない HTTP レスポンスを表示します。
...
<処理概略>
	...
	2. sceHTTPSetOption() でプロキシサーバの URL を設定する
	3. 目的の URL に対してプロキシ経由で HTTP 接続し、HTTP レスポンスを
	   取得する
	4. HTTP レスポンスを表示する
