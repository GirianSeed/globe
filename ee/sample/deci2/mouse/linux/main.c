...
int connected2ee = 0;						// status for connection with EE
int sampleProtoOpened = 0;					// status for sample protocol

static PKTLIST *pl;
static char *bname;
static int   fdSock;						// file descriptor for socket
static int   fdChild;						// file descriptor for child process
...
// return pointer for the leading packet in the packet list
// if packet has the right status
...
// get packet from packet list and send it
static int SendPacket(int fd, PKT *pkt) {
	int r = write(fd, pkt->deci2, pkt->deci2->length);
...
static int RecvPacket(int fd, u_char *buf) {
	...
	// read rest of data
	if ((n = read(fd, buf+len, pkt->length-len)) < 0)
		return ShowError("read() failed");
	if (n != pkt->length-len) return ShowError("read() failed.");

	switch (pkt->protocol) {	// distribute to proper handler
...
static int MySelect(void) {
	...
	FD_SET(fdSock, &rfds);
	FD_SET(fdChild, &rfds);
	if (GetHeadPktByStat(PKT_SEND_READY|PKT_NEED_REPLY)) FD_SET(fdSock, &wfds);
	...
	if (FD_ISSET(fdChild, &rfds)) {					// check child process
...
// make packet, send packet, and wait replay
...
static void Exit(int s) {				// signal handler
...
int main(int ac, char *av[]) {
	...
	signal(SIGCHLD, Exit);				// trap child process die
	...
	switch ((r = fork())) {							// make child process
	...
	// make, send NETMP connect packet, and wait replay
	if (SendAndWaitReply(NetmpMakeConnect()) < 0) 
		return ShowError("faild to send connect packet");
	// make, send NETMP reset   packet, and wait replay
