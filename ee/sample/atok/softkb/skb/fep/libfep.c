...
static volatile int g_FepStop;					// ワーカスレッドの停止要求フラグ
...
#ifndef ATOK_NO_LINK
static int FepLoadConfig(const char *pszCfgFile);
#endif
...
//  const char *pszConfig     : コンフィグファイルのパス   へのポインタ
...
int Fep_GetNowAtokInput(void)
{
...

	{
		...
		sceAtokConfig cfg;
		cfg.Size = sizeof(sceAtokConfig);
		sceAtokGetConfig(&g_Atok,&cfg,SCE_ATOK_CONFIG_KAJIINPUT);
...
static void workerMain(int param __attribute__((unused)))
{
	...
	while(1)
	{
		...
		// スレッド停止フラグが設定されていたかどうかチェック
		if(g_FepStop)
		{
			// FEP停止フラグ設定されていたらループを終了
			break;
		}
...
static int FepLoadConfig(const char *pszCfgFile)
{
	...
	// atok.iniに記録されていない設定を、アプリケーションが望む値で初期化します
	{
		...
		cfg.MaxInputChars = 30;		// 入力文字数
									// 学習対象の文節数が最大30文節と成っている。
									// この制約の関係で、31文節目以降の記憶しておかなければならない
									// 候補選択状況が記憶されていなかった為
									// 現在は、30文節にならないように一度に30文字しか最大入力できない。
									// CAUTION : これはATOKエンジンのバグです。
	...
	cfg.Size = sizeof(sceAtokConfig);
	sceAtokSetConfig(&g_Atok, &cfg, SCE_ATOK_CONFIG_KAJIINPUT       |
