...
int mnUnload(int mode, int btn) {
	...
	if(curbtn) {
		...
		case 0:
			/* unload mcserv.irx */
			oldserv = sceSifSearchModuleByName("mcxserv");
			if(oldserv >= 0) {
			...
			/* unload mcman.irx */
			oldman = sceSifSearchModuleByName("mcxman");
			if(oldman >= 0) {
		...
		case 1:
			...
			newman = sceSifLoadModule("host:/usr/local/sce/iop/modules/mcxman.irx", 0, 0);
			newserv = sceSifLoadModule("host:/usr/local/sce/iop/modules/mcxserv.irx", 0, 0);
	...
	if(draw) {
		...
		sceDevConsPrintf(console, "&%d1. unload mcxserv.irx  ",
			!phase ? 4:5);
		if(phase > 0) {
			if(oldserv < 0) {
		...
		sceDevConsPrintf(console, "&%d   unload mcxman.irx   ",
			!phase ? 4:5);
		if(phase > 0) {
			if(oldman < 0) {
				sceDevConsPrintf(console, "&2not found");
		...
		sceDevConsPrintf(console, "&%d2. reload mcxman.irx   ",
			phase==1 ? 4:5);
		if(phase > 1) {
			if(newman > 0) {
		...
		sceDevConsPrintf(console, "&%d   reload mcxserv.irx  ",
			phase==1 ? 4:5);
		if(phase > 1) {
			if(newserv > 0) {
