...
libhttp を利用して HTTP サーバに接続し、
リダイレクトを指定された場合にはリダイレクト先へ再接続を行なうサンプル。

<サンプルの解説>
	本プログラムは libhttp の リダイレクト機能を利用して、
	HTTP サーバがリダイレクトを指定した場合にリダイレクト先に再接続を
	行なうサンプルです。
...
<ファイル>
	http_redirect.c        : メインプログラム

<起動方法>
	$ make                          : コンパイル

	$ dsedb -r run http_redirect.elf <URL> : 実行

	指定された URL に対して HTTP 接続を行ない、サーバからリダイレクトを
	指定された場合には、リダイレクト先へ再接続を行ないます。
...
<処理概略>
	...
	3. HTTP レスポンスにリダイレクトを示すコードが指定されていた
	   場合には、リダイレクト先へ再度 HTTP 接続し、HTTPレスポンスを
　　　　　 取得する
	4. 結果を表示する
