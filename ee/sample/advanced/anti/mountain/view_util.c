...
//Euler angle correction for Taylor expansion of sin/cos 
// theta: original angle
// theta_cos: corrected angle for cos expansion
// theta_sin: corrected angle for sin expansion
void Euler(float theta[3], float theta_cos[3], float theta_sin[3])
{
	int	i;
	float 	PAI= 3.14159f;
...
//Make rotation matrix from Euler angle
// r: Euler angle
// mtx: rotation matrix
void RotMatrix(float r[3], float mtx[4][4])
{
float c0,c1,c2;
float s0,s1,s2;
...
//Make rotation matrix from rotation axis(normalized)
// phs: rotation angle around the rotation axis
// (x,y,z): roation axis
// rmat: rotation matrix
void get_rotmatrix_from_axis(float phs, float z, float y, float x, float rmat[4][4])
{
	float	az,ay,ax;
	float	amat[4][4];
	float	iamat[4][4];
...
//Make Screen/World matrix from view pointt and target point
// view: view point
// target: target point
// swmtx: Screen/World matrix
void get_SWmatrix_from_view(float view[3], float target[3], float swmtx[4][4])
{
...
// target: target point
// swmtx: Screen/World matrix
void get_VuSWmatrix_from_view(float view[3], float target[3], float swmtx[4][4])
{
	float	v[4] __attribute__((aligned(16)));
	float	bn;
