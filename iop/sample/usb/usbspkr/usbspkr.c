 ...
 *
 * $Id: usbspkr.c,v 1.7 2003/06/06 13:35:15 sonoda Exp $
 */
...
ModuleInfo Module = { "SCE_USB_SPEAKER_SAMPLE", 0x0102 };
...
#ifdef __MWERKS__
#define inform(...)	(printf("usbspkr,%s: ", __FUNCTION__), \
			 printf(__VA_ARGS__), -1)
#else
...
static void output_transfer(int fno){
...
#if defined(USE_MULTI_ISOCH_TRANSFERS)
	{
		...
		req.num_packets = FRAME_PER_REQ;

		for(total = i = 0; i < FRAME_PER_REQ; i++){
			n = rate_base;
...
static void thread(u_long arg){
	...
	buf_base = AllocSysMemory(0, buf_siz , NULL);
...
#endif
        CpuSuspendIntr(&oldstat);
	fname = AllocSysMemory(0, strlen(av[1]) + 1, NULL);
	CpuResumeIntr(oldstat);
	if(NULL == fname){
