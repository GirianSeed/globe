 ...
 * ---------------- */
Bool
metaMsgCB (unsigned char metaNo, unsigned char *bf, unsigned int len,
	   unsigned int private_data)
{
    ...
    /* False を返すと modmidi 内で current_point の位置から長さ len の
     * データが読み飛ばされる
     * → それらのデータはこのコールバック関数内で処理されたものと
     *    して扱われる */
    // return (False);

    /* 通常は True を返し、modmidi 内で処理させる
...
repeatCB (sceMidiLoopInfo *pInf, unsigned int private_data)
{
    ...
    /* False を返すと今回、および以降、リピートが行われない */
    // return (False);

    /* 通常は True を返し、リピートを続行させる
     * (あるいはコールバックそのものを設定しない)。*/
    return True;
}
