...
libhttp を利用して、HTTP サーバに接続し受信した
レスポンスに対して MIME マルチパート解析を行なうサンプル。

<サンプルの解説>
	本プログラムは libhttp を利用して HTTP サーバに
	接続し、受信したレスポンスに対して MIME マルチパート解析を行なう
	サンプルです。
...
<ファイル>
	http_mime.c        : メインプログラム

<起動方法>
	$ make                          : コンパイル

	$ dsedb -r run http_mime.elf <URL> : 実行

	指定された URL に対して HTTP 接続を行ない、取得した HTTP レスポンスに
	対して MIME マルチパート解析を行ない、解析結果の最初のパートを
	表示します。
...
<処理概略>
	...
	2. 目的の URL に対して HTTP 接続し、HTTP レスポンスを取得する
	3. MIME マルチパートフィルタを作成し、HTTP レスポンスに対して
	   フィルタ処理を再帰的に行なう
	4. 解析によって取得した最初のパートを表示する
