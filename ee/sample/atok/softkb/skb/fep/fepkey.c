...
static int FepTreatKeyAtok(sceAtokContext *pAtok, u_char led, u_char mkey, u_short rawcode)
{
	...
	else
	{
		...
		// テンキー入力が半角指定の時に、テンキーからの入力キャラクタが来たとき
		else if( (g_Fep.TenkeyInput==0 || (g_Fep.TenkeyInput==1 && g_Fep.KanDspType>=4))
		        && (usCharCode & (USBKB_RAWDAT|USBKB_KEYPAD))==USBKB_KEYPAD
		        && s==SCE_ATOK_ISTATE_BEFOREINPUT
		){
			f = usCharCode | RAW_CODE;	//ATOKに渡さず、直接入力。
		}
		else
		{
			if(!(mkey & (USBKB_MKEY_L_CTRL | USBKB_MKEY_R_CTRL)) &&
				...
				!((0x402C<=usCharCode) && (usCharCode<=0x402F)))
			{
				...
				usCharCode &= (~(USBKB_RAWDAT|USBKB_KEYPAD));	// USBKB 関係のフラグを削除。
