...
#if USE_ERX==0 && USE_OVERLAY==0
#define STANDALONE		1
#endif	// USE_ERX==0 && USE_OVERLAY==0
...
// 
#if USE_ERX
int main_overlay(char *filename)
#elif USE_OVERLAY
int main_ezmpeg(char *filename)
#else
...
static void playMpegFile(char *filename)
{
    ...
    // 
#if STANDALONE==0
    hand = AddIntcHandler(INTC_VBLANK_S, vblankHandler, 0);
#else
...
#if STANDALONE==0
    RemoveIntcHandler(INTC_VBLANK_S, hand);
#endif
...
// 
static int playMpeg(sceMpeg *mp, u_long128 *bs, int size)
{
    int ret;
    int drawid = 1;
    sceGsDispEnv disp;
	...
	if(AutoStep()) {
		paddata |= SCE_PADselect;
		printf("*** return from child ezmpeg ***\n");
	}
