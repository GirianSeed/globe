...
Sample program to connect to an HTTP server via a proxy server 
using libhttp

< Description of the Sample >
	This sample program connects to an HTTP server via a proxy server 
	using the proxy feature of libhttp.
...
< Activating the Program >
	$ make                          : Links libhttp by compiling 
					  the program
	$ dsedb -r run http_proxy.elf <URL> <proxy_URL> : Executes 
							  the program

	This program establishes an HTTP connection with a specified URL 
	and displays an HTTP response, using "proxy_URL" as a URL of a 
	proxy server.
...
< Operation Overview >
	...
	2. Uses sceHTTPSetOption() to specify a URL of a proxy server.
	3. Establishes an HTTP connection with the target URL via 
	   the proxy server, and receives an HTTP response.
