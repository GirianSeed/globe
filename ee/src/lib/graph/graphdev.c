...
void sceGsPutDispEnv(sceGsDispEnv *disp)
{
    ...
    gp = sceGsGetGParam();
    if(gp->sceGsVersion == 1){
		DPUT_GS_PMODE(*(u_long *)&disp->pmode);   //PMODE
		DPUT_GS_DISPFB1(*(u_long *)&disp->dispfb);  //DISPFB2
		DPUT_GS_DISPLAY1(*(u_long *)&disp->display); //DISPLAY2
		DPUT_GS_EXTDATA(*(u_long *)&disp->bgcolor); //BGCOLOR
