 ...
 *                        All Rights Reserved.
 */
// Last Modified on 03/24/03 19:10:00 sonoda
...
static void mainthread(void) {
    ...
    if ((r = ShowPartition("hdd0:")) < 0) {
	printf("cannot list partition\n");
	return;
    }
    ShowFree();
...
int start(int argc, char **argv) {
    ...
    //char dev9arg[] = "-sa" "\0" "SA_THFIFO";
    char dev9arg[] = "-sa" "\0" "SA_THPRI";
    char hddarg[] = "-o" "\0" "4" "\0" "-n" "\0" "10";
    char pfsarg[] = "-m" "\0" "4" "\0" "-o" "\0" "10" "\0" "-n" "\0" "24";

    if (!sceCdInit(SCECdINoD)) return 1;

    if ((r = LoadStartModule(MODPATH "dev9.irx", sizeof(dev9arg),
						     dev9arg, &rr) < 0)) {
