 ...
 *       0.5.0     Aug,03,2001     sasaki            Add comments, etc.
 */

//描画を 640x448 PSMCT16 + dither で行い、上下を 1/2 に圧縮して
//640x224 PSMCT32 の表示バッファに出力する
#include <eekernel.h>
#include <stdlib.h>
#include <stdio.h>
#include <math.h>
...
#define ANGLE_DELTA 0.2f
	...
	//描画用のバッファと表示用のバッファを固定にします。
	//画面を圧縮する時に転送をするので、ダブルバッファを入れかえる
	//必要はありません。
...
//                sceGsSetHalfOffset((frame&1) ? &db.draw1 : &db.draw0, 2048, 2048, odev);
		...
		//ダブルバッファの時は ODD, EVEN に合わせて描画原点を上下に半ピクセル移動しました。
		//このプログラムではバッファを圧縮する時に、圧縮前の画像の原点を１ピクセルずらします。
		//圧縮は、Vブランクの間に行います。Vsync の直後のこの辺りは、Vブランク期間です。
