...
int mpegBackground(sceMpeg *mp, sceMpegCbData *cbdata, void *anyData);
    ...
    int tagSize, TimeStamp *pts, int n_pts)
{
    ...
    sceMpegAddCallback(&vd->mpeg,
    	sceMpegCbBackground, mpegBackground, NULL);
...
int decBs0(VideoDec *vd)
{
	...
	if (mp->frameCount == 0) {
	    ...
	    disp_x = (MAX_WIDTH - mp->width) >> 1;
	    disp_y = (MAX_HEIGHT - mp->height) >> 2;
...
// ////////////////////////////////////////////////////////////////
//
//  Callback function for sceMpegCbBackground
//
int mpegBackground(sceMpeg *mp, sceMpegCbData *cbdata, void *anyData)
{
    proceedAudio();
