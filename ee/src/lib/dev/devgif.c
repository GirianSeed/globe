...
static int sceDevGifSync(void){
  ...
  if(stat & 0x00000008){ return 1; }		/*	PSE			*/
  if((stat & 0x00000fc0) != 0){ return 2; }		/*	‚È‚ñ‚ç‚©‚Ìƒf[ƒ^‚ ‚è	*/
  return 0;
}
...
int sceDevGifPutFifo(u_long128 *addr, int n){
  ...
  while(n--){
    if(DGET_GIF_STAT() & 0x10000000){ return count; }	/*	fifo full? */
    count++;
    DPUT_GIF_FIFO(*addr++);
  }
  return count;
