 ...
 *
 * $Id: ping.c,v 1.18.2.2 2003/10/31 07:24:34 ksh Exp $
 */
...
ModuleInfo Module = { "INET_ping_client", ((1 << 8) | 93) };

#define DEFAULT_TTY_NUMBER	1
...
static void ping_abort(u_long arg){
	...
	//sceInetAbort(1234, sceInetAbortResolver);
	sceInetAbort(0, sceInetAbortResolver);
	while(1)
		SleepThread();
}
...
#if 1
	if(0 > (r = sceInetName2Address(sceInetN2AF_Detailed, &com->hostaddr, av[0],
			RES_TIMEOUT, RES_RETRY))){
#else
	if(0 > (r = sceInetName2Address(sceInetN2AF_Abort,
			&com->hostaddr, av[0],
			RES_TIMEOUT, RES_RETRY, 1234))){
#endif
