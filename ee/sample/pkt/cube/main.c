 ...
 *  --------------------------------------------------------------------
 *      1.10            July,21,1999     ywashizu    add recognizing GsVersion
 */
...
int main(){
...

    {
      ...
      sceGifPkOpenGifTag(&packetGS, *(u_long128*)&giftag0);      sceGifPkAddGsAD(&packetGS, SCE_GS_ALPHA_1, SCE_GS_SET_ALPHA(0,1,0,1,0));
	...
	const u_long giftag_wall[2] = { SCE_GIF_SET_TAG(0, 1, 0, 0, 1,  4),
					0x0000000000005510L };
	const u_long giftag_bug[2]  = { SCE_GIF_SET_TAG(0, 1, 0, 0, 0,  1),
					0x000000000000000eL };
	u_int j;
	
	// read pad information
	sceRead(fd, &pad, sizeof(pad));
...
#if 1
	// add these 3 packets because of bugs in GS
	if(GSbugflg == 1){
	    sceVif1PkOpenGifTag(&packet, *(u_long128*)&giftag_bug);
	    sceVif1PkAddGsAD(&packet,
			     SCE_GS_PRMODECONT, SCE_GS_SET_PRMODECONT(0));
	    sceVif1PkAddGsAD(&packet, SCE_GS_PRMODE, 
			     SCE_GS_SET_PRMODE(0, 0, 0, 0, 0, 0, 0, 0));
	    sceVif1PkAddGsAD(&packet,
			     SCE_GS_PRMODECONT, SCE_GS_SET_PRMODECONT(1));
