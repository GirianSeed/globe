...
<Description of the Sample>
	...
	the object with the display area using the boundary box, clip
	by the polygon with the drawing area of GS and switch the back
	face clipping by polygon.
...
<Using the Controller>
	...
	Left stick	:	Translation around x and y axis 
				(viewpoint)

	SELECT	:	Switching of the object (CUBE/TORUS)
	START	:	On/off switching of the object clip
	Left stick:	On/off switching of the clip with the GS drawing 
			area by pushing the left stick (*1)
	Right stick:	Near/Far switching of the object clip side by 
			pushing the right stick

<Note>
	For this sample program, execute it with the ANALOG mode switch 
	on.

	(*1) Protrude the object from the screen widely, or go beyond 
	the drawing area of GS using the R2 button with the object clip 
	off. If the left stick is switched to on, protruded polygon is
	clipped.
