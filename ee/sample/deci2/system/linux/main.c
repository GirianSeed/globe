...
#define PROTO_SAMPLE	0xe000				// protocol number, this sample uses
...
static void ProcessNetmp(char *buf) {				// handle NETMP packets
...
static void ProcessDcmp(char *buf) {				// handle DCMP packets
...
static void ProcessSample(int fd, char *buf) {	// handle this sample protocol
	FILE *tfile;
	char *tname;
	char c;
	u_char *data;
	...
	result = (u_int *)(hdr + 1);				// pointer for result code
	data = (u_char *)(result + 1);				// pointer for data

	if ((tname = tmpnam(NULL)) == NULL) {		// create temporary file
	...
	strcat(data, ">");
	strcat(data, tname);
	*result = system(data);						// execute command

	// read result of system command from temporary file
	tfile = fopen(tname, "r");
    ...
    SendPacket(fd, buf, len);					// send packet
}
...
int main(int ac, char *av[]) {
	...
	if ((fd = ConnectNet(hostname)) < 0) {			// connect to server
		printf("%s: cannot connect to host\n", baseName);
		return 1;
	}

	if (SendNetmpConnect(fd) < 0) {					// send NETMP connect
	...
	while ((r = PacketComing(fd, &timeout)) >= 0) { // waiting packet
		if (r == 0) {								// wait timeout
