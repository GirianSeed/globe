...
Sample program that uses libhttp to reflect a cookie when connecting to an HTTP server that specifies a cookie. 

<Description>
This is a sample program that uses the cookie function of libhttp to reflect a cookie when connecting to an HTTP server that specifies a cookie.
...
<Run Method>
	...
	$ dsedb -r run http_cookie.elf <URL>	: Execute

When an HTTP connection is made to the specified URL and a cookie is specified from the server, that cookie is reflected in an HTTP request and a reconnection is performed.
...
This program runs under the condition that exactly one target device is Note that multiple connections from the same device are also not considered.
...
3. If a cookie had been specified in the HTTP response, reflect the cookie in an HTTP request, make the HTTP connection again, and obtain an HTTP response.
