...
libnetライブラリ上で動作するechoサーバサンプル


<サンプルの解説>
	このプログラムは、eeからlibnetを用いてTCPのpassive openを
	行ない、接続してきたクライアントから受信したデータをそのまま
	クライアントに送信するechoサーバプログラムです。
	同時に起動する待ち受けスレッド数を引数で指定できます。
...
<起動方法>
        ...
        $ make                                  : コンパイル
        $ dsedb	                                : dsedbを起動
        > run main.elf <th_num>                 : main.elfの実行。
		<th_num>: 同時に確立する待ち受けスレッド数
...
<処理概略>
        ...
        1. ワーカスレッドを起動する
	2. 通信のための初期化を行なう
        3. th_num個の待ち受けスレッドを起動する
	   待ち受けスレッドは、コネクションが確立する度に新たに生成される。
	   各待ち受けスレッドでは
        4. TCPの7番ポートに対してpassive openを行ない、
	   クライアントからの接続を待つ
        5. コネクションが確立したら、クライアントからのデータを待つ
        6. クライアントからのデータを受信すると、受信データをそのまま
	   クライアントに送信する
	7. クライアントからコネクションが切断されると終了
