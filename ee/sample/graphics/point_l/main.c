	...
	*(u_long*)&texenv.gs_tex.tex11 = SCE_GS_SET_TEX1(1,0,1,1,0,0,0);
	...
	sceGsSyncPath(0, 0);
	 ...
	 // --- object rotate & change view point ---
         if(paddata & SCE_PADRdown){ plight0[2]-=0.15;}
	 if(paddata & SCE_PADRup){ plight0[2]+=0.15;}
	 if(paddata & SCE_PADRright){ plight0[0]+=0.15;}
	 if(paddata & SCE_PADRleft){ plight0[0]-=0.15;}
	 if(paddata & SCE_PADR1){ 
	   plight0[1]+=0.15;
	   if(plight0[1]>-1.0) plight0[1]-=0.15; }
	 if(paddata & SCE_PADR2){ plight0[1]-=0.15;}

         if(paddata & SCE_PADLdown){ plight1[2]-=0.15;}
	 if(paddata & SCE_PADLup){ plight1[2]+=0.15;}
	 if(paddata & SCE_PADLright){ plight1[0]+=0.15;}
	 if(paddata & SCE_PADLleft){ plight1[0]-=0.15;}
	 if(paddata & SCE_PADL1){ 
	   plight1[1]+=0.15;
	   if(plight1[1]>-1.0) plight1[1]-=0.15; }
	 if(paddata & SCE_PADL2){ plight1[1]-=0.15;}
	 ...
	 *D1_QWC  = 0x00;
	 if(micro_switch==0)
	   *D1_TADR = (u_int)&My_plight_code & 0x0fffffff;
	 else *D1_TADR = (u_int)&My_plight2_code & 0x0fffffff;
	 *D_STAT = 2;
	 FlushCache(0);
	 *D1_CHCR = 1 | (1<<2) | (0<<4) | (1<<6) | (0<<7) | (1<<8);
	 ...
	 sceVu0CopyVector(vu1env->Point_light0, plight0);
	 sceVu0CopyVector(vu1env->Point_light1, plight1);
	 sceVu0CopyVector(vu1env->Point_color0, pcolor0);
	 sceVu0CopyVector(vu1env->Point_color1, pcolor1);
...
void DrawLightPoint(Vu1DmaPacket *dmavu1)
{
    ...
    *D1_CHCR = 1 | (1<<2) | (0<<4) | (1<<6) | (0<<7) | (1<<8);
    ...
    pObj = &My_cube_start;
    AddVu1ObjPacket(dmavu1, pObj, obj_rot, plight0);
    AddVu1ObjPacket(dmavu1, pObj, obj_rot, plight1);
