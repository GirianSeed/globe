...
<起動方法>
	...
	$ dsedb -r run http_blk.elf command <URL> [post-data] : 実行
	...
	command は HTTP メソッドを指定するもので、以下の3つを指定できます。
		...
		-head	HEAD メソッド
		-post	POST メソッド
	POST メソッドの場合のみ、post-data として POST するデータを
	指定できます。
...
<処理概略>
	...
	2. sceHTTPSetOption() でブロッキングを指定する
