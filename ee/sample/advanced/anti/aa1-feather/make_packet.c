...
//Scratch PAD を半分に分け、id でどちらかのアドレスを返します。
...
extern unsigned int My_refract_end __attribute__((section(".vudata")));
extern unsigned int My_lattice_end __attribute__((section(".vudata")));
...
void make_feather_packet(int *qq, int *rr, int rept, int prim_flg, int aa1_flg)
{
	...
	//メインメモリー上の三角板の中心位置データは連続的にキャッシュ経由で読み込みます。
	//三角板の回転姿勢データは Scratch PAD 上に計算されて載っています。
	//中心位置データに回転姿勢データを加え、DMA パケットのデータ部分に書き込みます。
	//書き込みの時は、Uncache Accellerated を使うことでキャッシュを汚すことを防ぎます。
...
float	target[3];
{
	...
	/*       OFY=2048						*/
	/*       Zmin=0							*/
	/*  Zmax=16000000.0 (2^24=16777216にすると計算誤差でoverflowする)*/
	/*								*/
...
//描画プリミティブを変えることで、クリアのオン、オフをしています。
