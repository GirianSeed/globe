...
static void gsctx_init(sceHiGsMemTbl *tbl, int width, int height)
{
    ...
    if(port0 == NULL) error_handling(__FILE__,__LINE__,sceHiGetErrStatePtr()->mes);
    ...
    if(port1 == NULL) error_handling(__FILE__,__LINE__,sceHiGetErrStatePtr()->mes);
    ...
    if(flush == NULL) error_handling(__FILE__,__LINE__,sceHiGetErrStatePtr()->mes);
}
