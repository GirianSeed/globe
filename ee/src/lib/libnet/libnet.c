 ...
 * INET
 **************************************/
PUBLIC	int 	sceInetPoll( sceSifMClientData *cd, u_int *net_buf, sceInetPollFd_t *fds, int nfds, int ms )
{
	int*	buf = ( int* )net_buf;
...
PUBLIC	int 	sceInetCtlGetState( sceSifMClientData *cd, u_int *net_buf, int id, int *pstate )
{
	int*	buf = ( int* )net_buf;
	int 	ret;
...
PUBLIC	int 	sceLibnetInitialize( sceSifMClientData *cd, u_int buffersize, u_int stacksize, int priority )
{
	...
	if ( buffersize != sceLIBNET_PAD( buffersize ) ) {
		scePrintf( "sceLibnetInitialize() buffersize\n" );
		return( sceLIBNETE_INSUFFICIENT_RESOURCES );
	}
...
PUBLIC	int 	sceLibnetWaitGetInterfaceEvent( sceSifMClientData *cd, u_int *net_buf, int *id, int *type )
{
	int*	buf = ( int* )net_buf;
	int 	ret;
