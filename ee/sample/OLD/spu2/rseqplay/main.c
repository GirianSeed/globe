...
int dataset(void)
{
	...
	i = sceSifLoadIopHeap( SEQ_FILENAME, (void*)gDataTopAddr);
	if( i < 0 ) { printf( "\nCan't load seq file to iop heap  %d\n", i ); 
		return -1; }
	i = sceSifLoadIopHeap( VH_FILENAME, (void*)(VH_ADDR_OFST+gDataTopAddr) );
	if( i < 0 ) { printf( "\nCan't load vh file to iop heap %d\n", i ); 
		return -1; }
	i = sceSifLoadIopHeap( VB_FILENAME, (void*)(VB_ADDR_OFST+gDataTopAddr) );
	if( i < 0 ) { printf( "\nCan't load vb file to iop heap %d\n", i ); 
		return -1; }
...
{
    sceSifInitRpc(0);
    if(sceSifLoadModule("host0:../../../..//iop/modules/rspu2drv.irx",
			0, NULL) < 0){
