...
// --- added by sasaki ---
extern sceVif1Packet	MyVif1Packet;	//Packet Library がデータを生成する pointer 等を含んだ構造体
					//先ずデータを MyVif1Packet を用いて生成し、その後構造体を
					//MyVif1Packet1 等にコピーすることにより、複数のデータを
					//保存する。詳しくはパケットライブラリーを参照のこと。
...
void initialize_packet_buffer1()
{
	...
	//パケットデータを生成するポインターを PacketData1 に設定する
	sceVif1PkInit(&MyVif1Packet, PacketData1);
	sceVif1PkReset(&MyVif1Packet);

	sceVif1PkAddCode(&MyVif1Packet, SCE_VIF1_SET_FLUSH(0));
...
//配列に入ったポリゴンメッシュデータを、MyVif1Packet の示すデータ格納場所に
//Triangle Strip データとして格納する。
//ndata: 配列の縦横の次数
//mpos: 位置データ
//mnorm: 法線データ
...
//	    pData[3] = 0x00000004; pData[2] = 0x12412412; pData[1] = 0x902dc000; pData[0] = 0x00008000 + n_poly;//ABE+TME
	...
	vx = 100.0f; vy = 100.0f; vz = 200.0f; va = 80.0f;
	pData[0] = *(u_int*)&vx; pData[1] = *(u_int*)&vy; pData[2] = *(u_int*)&vz; pData[3] = *(u_int*)&va;
	sceVif1PkAddUpkData32N(&MyVif1Packet, pData, 4);
...
//配列に入ったポリゴンメッシュデータを、MyVif1Packet の示すデータ格納場所に
//Line Strip データとして格納する。
//
//Line は縦、横のメッシュ状に配置され、シェーディング用の Triangle Strip データとは
//異なる配置をとる。
//
//ポリゴンの各頂点に集まる AA1 Line の数が異なると、αブレンドの回数が変わって
//しまうため、エッジの頂点が角張って見えたりする。このため、ポリゴンの対角線に
//AA1 Line を引くと滑らかなアンチエイリアシングが出来ない。
//
//ndata: 配列の縦横の次数
//mpos: 位置データ
//mnorm: 法線データ
...
//３次のBezier 曲線の内部の２つの制御点を、両端点の位置および法線ベクトルから求める。
//ある程度美しい形状になるように適切な制御点を求める。
...
//３次のBezier 曲線の内部の２つの制御点を、両端点の位置および接線ベクトルから求める。
//ある程度美しい形状になるように適切な制御点を求める。
...
//4角形ポリゴンの4頂点の位置と法線ベクトルから、内部を滑らかに膨らませた曲面メッシュを生成する。
//結果はポリゴンメッシュとして配列 pos & nrm に保存される。
...
//4角形ポリゴンの4頂点の位置と法線ベクトルから、内部を滑らかに膨らませた曲面メッシュを生成する。
//また、2辺に関しては接線ベクトル in_dir も指定出来る。
//結果はポリゴンメッシュとして配列 pos & nrm に保存される。
...
void create_cup()
{
	...
	// outer shell
	in_pos[0][1][0] =  0.0f; in_pos[0][1][1] =  4.0f; in_pos[0][1][2] =  2.0f;
	in_pos[0][0][0] =  0.0f; in_pos[0][0][1] = -4.0f; in_pos[0][0][2] =  4.0f;
	in_pos[1][1][0] =  2.0f; in_pos[1][1][1] =  4.0f; in_pos[1][1][2] =  0.0f;
	in_pos[1][0][0] =  5.0f; in_pos[1][0][1] = -4.0f; in_pos[1][0][2] =  0.0f;

	in_nrm[0][1][0] =  0.0f; in_nrm[0][1][1] =  1.0f; in_nrm[0][1][2] =  1.0f;
	in_nrm[0][0][0] =  0.0f; in_nrm[0][0][1] =  0.0f; in_nrm[0][0][2] =  1.0f;
	in_nrm[1][1][0] =  1.0f; in_nrm[1][1][1] =  1.0f; in_nrm[1][1][2] =  0.0f;
	...
	in_pos[0][0][0] =  -0.0f; in_pos[0][0][1] = -4.0f; in_pos[0][0][2] =  4.0f;
	in_pos[1][1][0] =  -2.0f; in_pos[1][1][1] =  4.0f; in_pos[1][1][2] =  0.0f;
	in_pos[1][0][0] =  -4.0f; in_pos[1][0][1] = -4.0f; in_pos[1][0][2] =  0.0f;
	...
	// bottom
	in_pos[0][0][0] =  0.0f; in_pos[0][0][1] =  4.0f; in_pos[0][0][2] =  2.0f;
	in_pos[1][0][0] =  2.0f; in_pos[1][0][1] =  4.0f; in_pos[1][0][2] =  0.0f;
	in_pos[0][1][0] =  0.0f; in_pos[0][1][1] =  4.8f; in_pos[0][1][2] =  0.0f;
	in_pos[1][1][0] =  0.0f; in_pos[1][1][1] =  4.8f; in_pos[1][1][2] =  0.0f;

	in_nrm[0][0][0] =  0.0f; in_nrm[0][0][1] =  1.0f; in_nrm[0][0][2] =  1.0f;
	in_nrm[1][0][0] =  1.0f; in_nrm[1][0][1] =  1.0f; in_nrm[1][0][2] =  0.0f;
	...
	// outer edge
	in_pos[0][0][0] =  0.0f; in_pos[0][0][1] = -5.0f; in_pos[0][0][2] =  3.0f;
	in_pos[1][0][0] =  3.0f; in_pos[1][0][1] = -5.0f; in_pos[1][0][2] =  0.0f;
	in_pos[0][1][0] =  0.0f; in_pos[0][1][1] = -4.0f; in_pos[0][1][2] =  4.0f;
	in_pos[1][1][0] =  5.0f; in_pos[1][1][1] = -4.0f; in_pos[1][1][2] =  0.0f;
	...
	in_dir[0][0][0] =  1.0f; in_dir[0][0][1] =  0.0f; in_dir[0][0][2] =  0.0f;
	in_dir[1][0][0] =  0.0f; in_dir[1][0][1] =  0.0f; in_dir[1][0][2] = -1.0f;
	in_dir[0][1][0] =  1.0f; in_dir[0][1][1] =  0.0f; in_dir[0][1][2] =  0.0f;
	in_dir[1][1][0] =  0.0f; in_dir[1][1][1] =  0.0f; in_dir[1][1][2] = -1.0f;
	...
	in_pos[1][0][0] =  -3.0f; in_pos[1][0][1] = -5.0f; in_pos[1][0][2] =  0.0f;
	in_pos[0][1][0] =   0.0f; in_pos[0][1][1] = -4.0f; in_pos[0][1][2] =  4.0f;
	in_pos[1][1][0] =  -4.0f; in_pos[1][1][1] = -4.0f; in_pos[1][1][2] =  0.0f;
...
void create_handle()
{
	...
	pos[0][0] = 2.0f; pos[0][1] = 2.0f; pos[0][2] = 0.0f;
	dir[0][0] = 1.0f; dir[0][1] = 0.0f; dir[0][2] = 0.0f;

	pos[1][0] = 7.0f; pos[1][1] =-2.0f; pos[1][2] = 0.0f;
	dir[1][0] = 0.0f; dir[1][1] =-1.0f; dir[1][2] = 0.0f;

	create_tube(0.6f, pos, dir);

	pos[0][0] = 7.0f; pos[0][1] =-2.0f; pos[0][2] = 0.0f;
	dir[0][0] = 0.0f; dir[0][1] =-1.0f; dir[0][2] = 0.0f;

	pos[1][0] = 5.0f; pos[1][1] =-4.5f; pos[1][2] = 0.0f;
	dir[1][0] =-1.0f; dir[1][1] = 0.0f; dir[1][2] = 0.0f;

	create_tube(0.6f, pos, dir);

	pos[0][0] = 5.0f; pos[0][1] =-4.5f; pos[0][2] = 0.0f;
	dir[0][0] =-1.0f; dir[0][1] = 0.0f; dir[0][2] = 0.0f;

	pos[1][0] = 3.0f; pos[1][1] =-3.0f; pos[1][2] = 0.0f;
	dir[1][0] = 0.0f; dir[1][1] = 1.0f; dir[1][2] = 0.0f;
