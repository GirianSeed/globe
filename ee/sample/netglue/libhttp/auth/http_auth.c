...
int main(int argc, char **argv){
	...
	if(argc < 4){
		printf("usage: http_auth.elf url username passwd\n");
		ERR_STOP;
	}
	...
	case sceHTTPC_Unauthorized:
		...
		if (alist == NULL){
			printf("Can't get authenticate info\n");
			ERR_STOP;
		}

		if (sceHTTPCleanUpResponse(client) < 0){
			printf("sceHTTPCleanUpResponse failed\n");
			ERR_STOP;
		}
		...
		if (alist->auth.type == sceHTTPAuth_BASIC){
			if (sceHTTPSetBasicAuth(client, argv[2], argv[3], 0) < 0){
				printf("sceHTTPSetBasicAuth failed\n");
				ERR_STOP;
			}
		} else {
			int qop;
			dig.username = argv[2];
			dig.password = argv[3];
			dig.realm = alist->auth.realm;
			dig.uri = p->filename;
			dig.nonce = alist->auth.nonce;
			dig.opaque = alist->auth.opaque;
			...
			if (dig.qop){
				int rnd = (int)response->date + rand();
			...
			if (sceHTTPSetDigestAuth(client, &dig, 0) < 0){
				printf("sceHTTPSetDigestAuth failed\n");
				ERR_STOP;
			}
		}
		...
		if (sceHTTPRequest(client) < 0){
			printf("2nd sceHTTPRequest failed\n");
			ERR_STOP;
		}
		...
		if (infop){
			...
			case 1:
				printf("<<Authentication-Info verified>>\n");
				break;
			case 0:
				printf("<<Authentication-Info not verified>>\n");
				break;
			case -1:
			default:
				printf("sceHTTPVerifyAuthInfo failed\n");
				ERR_STOP;
				break;
			}
			if (sceHTTPFreeAuthInfo(infop) < 0){
				printf("sceHTTPFreeAuthInfo failed\n");
				ERR_STOP;
			}
		}
		if (sceHTTPFreeAuthList(alist) < 0){
			printf("sceHTTPFreeAuthList failed\n");
			ERR_STOP;
		}
		break;
	default:
