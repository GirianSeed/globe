 ...
 *      1.01            Nov, 7,2001     iwano
 *      1.02            May,28,2003     iwano
 */
...
/* sceMcTblGetDir構造体構成メンバアクセス用マクロ */
...
void dispError(int ret) {
	...
	case -3:
		...
		break;
	case -4:
		sceDevConsMessage(console,15,10," Wrong Entry Name ");
		break;
	case -5:
		sceDevConsMessage(console,15,10," Permission Denied ");
		break;
	default:
...
int mnDefaultEnt(int mode, int btn) {
	...
	static sceVu0IVECTOR bgcolor[4] = {	/* 0xff が最大輝度 */
		{ 0x80,    0,    0, 0 },		/* 背景色(左上) */
		{    0, 0x80,    0, 0 },		/* 背景色(右上) */
		{    0,    0, 0x80, 0 },		/* 背景色(左下) */
...
/*#mnUnload --- unload memory card related iop modules */
int mnUnload(int mode, int btn) {
	static int oldbtn;
	static int phase, draw;
	int curbtn, result;
	static int oldserv, oldman, newserv, newman, retserv, retman;
	...
	if(curbtn) {
		...
		case 0:
			/* unload mcserv.irx */
			oldserv = sceSifSearchModuleByName("mcserv");
			if(oldserv >= 0) {
				retserv = sceSifStopModule(oldserv, 1, "", &result);
				if(retserv >= 0) {
					retserv = sceSifUnloadModule(oldserv);
				}
			}

			/* unload mcman.irx */
			oldman = sceSifSearchModuleByName("mcman");
			if(oldman >= 0) {
				retman = sceSifStopModule(oldman, 1, "", &result);
				if(retman >= 0) {
					retman = sceSifUnloadModule(oldman);
				}
			}

			if(retserv < 0 || retman < 0) {
				++phase;
			}
			break;
		case 1:
			/* reload mcman.irx, mcserv.irx */
			newman = sceSifLoadModule("host:/usr/local/sce/iop/modules/mcman.irx", 0, 0);
			newserv = sceSifLoadModule("host:/usr/local/sce/iop/modules/mcserv.irx", 0, 0);

			/* re-bind between EE-IOP needed */
			sceMcInit();
			break;
		case 2:
			sceDevConsClearBox(console, 0, MaxVertTitle+1 ,SizeConsX,20);
			return 0;
		}
	...
	/* display results */
	if(draw) {
		sceDevConsClearBox(console, 0, MaxVertTitle+1 ,60,20);
		sceDevConsLocate(console, 5, MaxVertTitle+ 3);
		sceDevConsPrintf(console, "&%d1. unload mcserv.irx  ",
			!phase ? 4:5);
		if(phase > 0) {
			if(oldserv < 0) {
				sceDevConsPrintf(console, "&2not found");
			}
			else {
				sceDevConsPrintf(console, "&7ID %d  ",oldserv);
				if(retserv >= 0) {
					sceDevConsPrintf(console, "succeded");
				}
				else {
					sceDevConsPrintf(console, "&2error %d", retserv);
				}
			}
		}
		sceDevConsLocate(console, 5, MaxVertTitle+ 4);
		sceDevConsPrintf(console, "&%d   unload mcman.irx   ",
			!phase ? 4:5);
		if(phase > 0) {
			if(oldman < 0) {
				sceDevConsPrintf(console, "&2not found");
				...
				sceDevConsPrintf(console, "&7ID %d  ", oldman);
				if(retserv >= 0) {
					sceDevConsPrintf(console, "succeded");
				}
				else {
					sceDevConsPrintf(console, "&2error %d", retman);
				}
			}
		}
		sceDevConsPrintf(console, "&7");

		sceDevConsLocate(console, 5, MaxVertTitle+ 6);
		sceDevConsPrintf(console, "&%d2. reload mcman.irx   ",
			phase==1 ? 4:5);
		if(phase > 1) {
			if(newman > 0) {
				sceDevConsPrintf(console, "&7ID %d", newman);
			}
			else if(newman < 0) {
				sceDevConsPrintf(console, "&2error %d",newman);
			}
			else {
				sceDevConsPrintf(console, "&2unchanged");
			}
		}
		sceDevConsLocate(console, 5, MaxVertTitle+ 7);
		sceDevConsPrintf(console, "&%d   reload mcserv.irx  ",
			phase==1 ? 4:5);
		if(phase > 1) {
			if(newserv > 0) {
				sceDevConsPrintf(console, "&7ID %d", newserv);
			}
			else if(newserv < 0) {
				sceDevConsPrintf(console,"&2error %d",newserv);
			}
			else {
				sceDevConsPrintf(console, "&2unchanged");
			}
		...
		sceDevConsLocate(console, 5, MaxVertTitle+ 10);
		sceDevConsPrintf(console, "push key to proceed");
