...
Sample program that uses libhttp to connect to an HTTP server for which HTTP authentication (BASIC authentication and digest authentication) is required.

<Description>
This sample program uses the HTTP authentication function of libhttp to connect to an HTTP server for which authentication is requested.
BASIC authentication and digest authentication are supported by libhttp, and this sample performs both types of requested authentication.
...
<Run Method>
	...
	$ dsedb -r run http_auth.elf <URL> <user> <passwd>: Execute

An HTTP connection is made to the specified URL. When the HTTP response indicates that authentication is required, create an HTTP request based on the user name <user> and password <passwd>, then make the connection again.
...
2. Make an HTTP connection to the target URL and obtain an HTTP response.
3. If "Not authenticated (401)" is returned as the HTTP response, create authentication data based on the user name <user> and password <passwd>, make the HTTP connection again, and obtain the HTTP response.
