...
Sample program to connect to an HTTP server and perform MIME multi-part 
parsing to a received response, using libhttp
 
< Description of the Sample >
	This sample program connects to an HTTP server and performs MIME 
	multi-part parsing to a received response, using libhttp .
...
< Activating the Program >
	$ make                          : Links libhttp by compiling 
					  the program
	$ dsedb -r run http_mime.elf <URL> : Executes the program

	This program establishes an HTTP connection with a specified URL, 
	performs MIME multi-part parsing to a received HTTP response, 
	and displays the first part of the parsing results.
...
< Operation Overview >
	...
	3. Creates a MIME multi-part filter, and performs filtering to 
	   the HTTP response recurrently.
	4. Displays the first part of the parsing results.
