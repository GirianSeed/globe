 ...
 *       1.00           Nov, 8,2000     kumagae     Optimized
...
KassWave関数が99%の演算時間を消費している部分です。よってチューニングの対象は
KassWave関数になるので、この関数で消費しているサイクル数を計測しました。

            CPU cycle    D-Cache Miss
0.初期状態   10.11M        154k
1.VER1        7.98M         91k
2.VER2        7.80M         76k 
3.VER3        7.66M         57k
...
1.DMAを使った転置行列計算
100*100*float型の行列の転置行列を求めています。
普通に転置を行なっていると、メモリレイアウト的に
縦方向にアクセスすることになり、Dキャッシュミスが
連続発生し、メモリネックになってしまいます。

そこで、DMAInterleaveのDMA転送を用いて
SPR上にもってきて、行列の転置をおこなうことで
Dキャッシュミスを起こさなくなります。

2.DMAを使ったmemcpy
DMAを使ったmemcpyです。
仕組みは、SPRを経由してDMA転送を行ないます。

3.SPRの利用
キャッシュミスを減らすためにSPRにDMA転送してから
演算を行ないます。演算後メインメモリへDMA転送します。

4.VU0macro
3.でSPR上に載せたデータをVU0macroをもちいて,
演算を行ないます。

*/

/*最適化バージョンの指定 指定がない時には,最適化なし*/
#define VER4
...
void DmaMemCopy(void *inDst, const void *inSrc, size_t inSize);
...
/* DmaMemCopy
 * SPRを経由したDMACopy,転送可能なqwcのサイズに制限はありません。
 * FlushCacheを行なっていないので,
 * 非キャッシュ領域でもちいるか、FlushCacheを呼び出す必要あり。
 *
 * void *outDst      転送先アドレス
 * const void *inSrc 転送元アドレス
 * size_t inSize     転送サイズ(qword単位)
 */
void DmaMemCopy(void *outDst, const void *inSrc, size_t inSize)
{
	const u_int QWC_SIZE = 1024;	//一度に送れるqwcサイズ
	const u_int spr = 0x70000000;	//SPRの開始アドレス

	u_int dst = (u_int)outDst;
	u_int src = (u_int)inSrc;
	u_int qwc = (u_int)inSize / 16;

	/* 1度に送れるDMA転送量はQWC_SIZE */
	while (qwc > QWC_SIZE) {
		/* SPRへのDMA転送 */
		*D_PCR = ~0x3ff;
		*D_STAT = 0x200;
...
void KassWave(float timestep)
{
...
#ifdef VER2
	FlushCache(0);
	DmaMemCopy(height_n_2, height_n_1, sizeof(height_n_1));
	DmaMemCopy(height_n_1, height_n, sizeof(height_n));
#else
...
#ifdef VER1
	FlushCache(0);
	TransporsedMatrix(height_n2, height_n);
	TransporsedMatrix(height_n_12, height_n_1);
	TransporsedMatrix(height_n_22, height_n_2);
#else
	for (i = 0; i < 100; i++) {
		for (j = 0; j < 100; j++) {
...
#ifdef VER2
	FlushCache(0);
	DmaMemCopy(height_n_22, height_n_12, sizeof(height_n_12));
	DmaMemCopy(height_n_12, height_n2, sizeof(height_n_12));
#else
...
#ifdef VER1
	FlushCache(0);
	TransporsedMatrix(height, height_n2);
	TransporsedMatrix(height_n, height_n2);
	TransporsedMatrix(height_n_2, height_n_22);
	TransporsedMatrix(height_n_1, height_n_12);
#else
	for (i = 0; i < 100; i++) {
		for (j = 0; j < 100; j++) {
	...
	for (i = 0; i < 100; i++) {
...
#ifdef VER3
		/*
		   VER3でのSPR上のメモリ使用レイアウト
		   0x70000000 +-----------------------------+
		   |psurface_polygon_left        |
		   0x70000640 +-----------------------------+
		   |psurface_polygon_right       |        
		   0x70000c80 +-----------------------------+
		   |psurface_polygon_normal_left |
		   0x700012c0 +-----------------------------+
		   |psurface_polygon_normal_right|
		   0x70001900 +-----------------------------+
		   |未使用                       |
		   0x70004000 +-----------------------------+

		 */
		sceVu0FVECTOR *psurface_polygon_left = (sceVu0FVECTOR*)0x70000000;
		sceVu0FVECTOR *psurface_polygon_right = (sceVu0FVECTOR*)0x70000640;
		sceVu0FVECTOR *psurface_polygon_normal_left = (sceVu0FVECTOR*)0x70000c80;
		sceVu0FVECTOR *psurface_polygon_normal_right = (sceVu0FVECTOR*)0x700012c0;

		*D_PCR = ~0x3ff;
		*D_STAT = 0x200;
		*D9_MADR = (u_int)&surface_polygon_left[i][0];
		*D9_QWC = 100;
		*D9_SADR = (u_int)0x70000000;
		*D_PCR |= 0x200;
		*D9_CHCR = 0x101;
		WaitDMA();
		...
		*D_PCR = ~0x3ff;
		*D_STAT = 0x200;
		*D9_MADR = (u_int)&surface_polygon_right[i][0];
		*D9_QWC = 100;
		*D9_SADR = (u_int)0x70000640;
		*D_PCR |= 0x200;
		*D9_CHCR = 0x101;
		WaitDMA();
#endif
		...
		for (k = 2; k <= 99; k++) {
			...
			__asm__ volatile (
				...
				"lqc2 vf2, 32(%2)\n"
				"lqc2 vf3, 16(%2)\n"
				"vsub.xyz vf4, vf1, vf2\n"
				"vsub.xyz vf5, vf3, vf2\n"
				"sqc2 vf4, 0(%0)\n"
				"sqc2 vf5, 0(%1)\n"
				: : "r" (work1), "r"(work2), "r"(psurface_polygon_left[k - 2]) : "memory"
			);
#else
			work1[0] = psurface_polygon_left[k - 2][0] - psurface_polygon_left[k][0];
			work1[1] = psurface_polygon_left[k - 2][1] - psurface_polygon_left[k][1];
			work1[2] = psurface_polygon_left[k - 2][2] - psurface_polygon_left[k][2];

			work2[0] = psurface_polygon_left[k - 1][0] - psurface_polygon_left[k][0];
			work2[1] = psurface_polygon_left[k - 1][1] - psurface_polygon_left[k][1];
			work2[2] = psurface_polygon_left[k - 1][2] - psurface_polygon_left[k][2];
#endif
#ifdef VER4
			__asm__ volatile (
				"lqc2 vf1, 0(%2)\n"
				...
				"sqc2 vf4, 0x0(%0)\n"
				"sqc2 vf5, 0x0(%1)\n"
				: : "r" (work4), "r"(work5), "r"(psurface_polygon_right[k - 2]) : "memory"
			);
#else
			// right lattice 
			work4[0] = psurface_polygon_right[k - 2][0] - psurface_polygon_right[k][0];
			work4[1] = psurface_polygon_right[k - 2][1] - psurface_polygon_right[k][1];
			work4[2] = psurface_polygon_right[k - 2][2] - psurface_polygon_right[k][2];

			work5[0] = psurface_polygon_right[k - 1][0] - psurface_polygon_right[k][0];
			work5[1] = psurface_polygon_right[k - 1][1] - psurface_polygon_right[k][1];
			work5[2] = psurface_polygon_right[k - 1][2] - psurface_polygon_right[k][2];
#endif
#else
			// left lattice 
			...
			memcpy(&psurface_polygon_normal_left[k], work3, sizeof(float) * 3);
			memcpy(&psurface_polygon_normal_right[k], work6, sizeof(float) * 3);
...
#ifdef VER3
		memcpy(&psurface_polygon_normal_right[0], &psurface_polygon_normal_left[99], sizeof(float) * 3);
		memcpy(&psurface_polygon_normal_right[1], &psurface_polygon_normal_left[98], sizeof(float) * 3);
		...
		*D_PCR = ~0x3ff;
		*D_STAT = 0x100;
		*D8_MADR = (u_int)surface_polygon_normal_left[i];
		*D8_QWC = 100;
		*D8_SADR = (u_int)0x70000c80;
		*D_PCR |= 0x100;
		*D8_CHCR = 0x100;
		WaitDMA();
		...
		*D_PCR = ~0x3ff;
		*D_STAT = 0x100;
		*D8_MADR = (u_int)surface_polygon_normal_right[i];
		*D8_QWC = 100;
		*D8_SADR = (u_int)0x700012c0;
		*D_PCR |= 0x100;
		*D8_CHCR = 0x100;
		WaitDMA();
#else
