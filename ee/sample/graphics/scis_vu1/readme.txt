...
<Description of the Sample>
	...
	Normally, this area should be set to the frame of (x, y)=(+/-2048) 
	...
	(eg.) ground, road, wall
        ...
        VU1's performance has been highly improved in comparison with
	CORE&VU0. The VU1code size has also become much larger.  Since the
	function branch by stack saving of the register value is processed
	in this version, this may be utilized as an example of the
	large-scale VU1 program.  
...
<File>
	...
	scissor.vcl	: VCL source code for generating VU1 micro code
	vcl_sml.i	: VCL macros needed for scissor.vcl
...
<Using the Controller>
	...
	L1/L2 button                  : Translation around z-axis  
		                   	(viewpoint, back and forth)
        Triangle/Eks button  : Modification of a clipping area 
			       (y direction)
        Square/Circle button : Modification of a clipping area 
			       (x direction)
        R1/R2  button     : Modification of a clipping area (xy direction)
        SELECT button     : Model switching (cube, torus)
        START button      : Switching the scissoring polygon display
                            (TriangleFan or LineStrip)

<Note>
	scissor.vcl is the VCL source code, which generates VU1 micro code 
        equivalent to scissor.vsm.  To compile scissor.vcl, VCL ver. 1.3 
	or later is required.  As compared to scissor.vsm, scissor.vcl is 
	easy to make changes to when using it with changes.

    Bug Fix

	In Release 2.4 and earlier versions, VU program included improper 
	code, which might cause a problem according to usage patterns. 
	Since GIFTAG areas for a dummy XGKICK instruction and a real 
	XGKICK instruction used the same memory address, using this sample 
	program in combination with a data transfer via PATH3 might 
	overwrite the data for the dummy XGKICK instruction before the 
	real GIFTAG was transferred.  This problem has been fixed in 
	Release 2.5. 
	With this change, the initial value of the stack for storing 
	registers in the code has been changed from the original 
	value 1024 to 1023, and the process to store the VF17 register 
	that does not need to be stored has been deleted.


    Improvement in 2nd Version
	
	In the previous version, slight gaps were generated between 
	scissoring polygons due to a computational error.  
	In this version, a method of avoiding this problem has been 
	adopted.  
	A code which slightly magnifies scissoring polygons has been 
	inserted immediately after the PUSHTRIANGLE: label in the source, 
	scissor. vsm.  Note that an adjustment of the magnification rate 
	1.01 might be required depending on the usage.       
	...
	representation by specific instruction such as a FTOI instruction.
	...
	sacrificing the Zbuffer width.  With the program like this sample,
...
The outline of the processing is as follows.
	...
	7. Obtains the vertex color from the inner product of the normal 
	   line vector.
	8. Calculates the texture coordinates (S,T,Q).
	...
	10. Branches to the scissoring routine (S1) if the applicable 
	    polygon is striding over the cut surface.
	...
	S2. Passes the received polygon (considered as TriangleFan) to the
	...
	T2. Registers the end information in the next TriangleFan list 
	    from the edge state. 
            - Do not register if the endpoint A and B of the edge are 
	      outer.
            - Registers if both endpoints of A and B of the edges are
   	      inner.
