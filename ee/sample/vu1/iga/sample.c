...
{
	int i; /* loop counter */
	int fd; /* id parameter for open function */
	u_int frame; /* frame number counter */
	int oddeven; /* field id flag(odd or even) */
	scePadData pad; /* area for controller */
	...
	while(1){
		...
		if(pad.button&SCE_PADLup) speed -= 5.0f;
		if(pad.button&SCE_PADLdown) speed += 5.0f;
		if(speed > 1000.0f) speed = 1000.0f;
		if(speed < -1000.0f) speed = -1000.0f;

		if(!(pad.button&SCE_PADk))
			MakePosition();
