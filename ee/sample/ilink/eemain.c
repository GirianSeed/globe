 ...
 *      2.0             2001.11.19      oonuki  unload module
 *  --------------------------------------------------------------------
 */
#define SEND_NO_WAIT
...
#define TX_SIZE		SCEILSOCK_MAX_PAYLOAD_SIZE // in bytes (= 488)
...
void sceSocketPrepare()
{
  ...
  th_RcvID = CreateThread( &pt );
  if( th_RcvID >= 0 )
  {
    StartThread( th_RcvID, NULL );
  } else {
...
void sceSocketClose()
{
  TerminateThread( th_RcvID );
  DeleteThread( th_RcvID );

  sceILsockReset();

  sceILsockClose( sdRx ); // for Receive
  sceILsockClose( sdTx ); // for Transmit
  sceILsockClose( sdBc ); // for Broadcast
}
...
int main(void)
{
  int i;
  int ret, result;
  int ilsample_id, ilsock_id, ilink_id;
  ...
  while(sceSifLoadModule( "host0:/usr/local/sce/iop/modules/padman.irx", 0, NULL ) < 0){
    scePrintf("Can't load module padman\n");
  }
  while((ilink_id = sceSifLoadModule( "host0:/usr/local/sce/iop/modules/ilink.irx", 0, NULL )) < 0 ){
    scePrintf( "Can't load module ilink\n" );
  }
  while((ilsock_id = sceSifLoadModule( "host0:/usr/local/sce/iop/modules/ilsock.irx", 0, NULL )) < 0){
    scePrintf("Can't load module ilsock\n");
  }
  while((ilsample_id = sceSifLoadModule( "host0:/usr/local/sce/iop/sample/ilink/ilsample.irx", 0, NULL )) < 0 ){
    scePrintf("Can't load module ilsample\n");
  }
  ...
  scePadInit(0);        /* コントローラライブラリ初期化 */
  if( scePadPortOpen( 0, 0, pad_dma_buf )==0 ){
    sprintf( errmsg, "ERROR: scePadPortOpen\n" );
    return 0;
  }
  ...
  while( 1 )
  {
    ...
    padon = pad & (~padb);      /* ボタンが押された時だけビットＯＮに */

    /* 終了処理を行って、ループを抜ける */
    if( padon & SCE_PADRright ) break;
...
retry:
  ...
  /* モジュールのストップとアンロード */
  ret = sceSifStopModule( ilsample_id, 0, NULL , &result );
  while( ret < 0 ){
    scePrintf( "Can't stop module ilsample\n" );
  }

  ret = sceSifUnloadModule( ilsample_id );
  while( ret < 0 ) {
    scePrintf("ilsample.irx: unload fail (%d)\n", ret );
  }

  ret = sceSifStopModule( ilsock_id, 0, NULL , &result );
  while( ret < 0 ){
    scePrintf( "Can't stop module ilsock\n" );
  }

  ret = sceSifUnloadModule( ilsock_id );
  while( ret < 0 ) {
    scePrintf("ilsock.irx: unload fail (%d)\n", ret );
  }

  ret = sceSifStopModule( ilink_id, 0, NULL , &result );
  while( ret < 0 ){
    scePrintf( "Can't stop module ilink\n" );
  }

  ret = sceSifUnloadModule( ilink_id );
  while( ret < 0 ) {
    scePrintf("ilink.irx: unload fail (%d)\n", ret );
  }
