...
Sample program to use libhttp to reconnect to the HTTP server 
where data is redirected if redirection is specified 

< Description of the Sample >
	This sample program uses the redirection feature of libhttp .
	If redirection is specified by an HTTP server, 
	the program establishes a reconnection with the HTTP server where 
	data is redirected.
...
< Activating the Program >
	$ make                          : Links libhttp by compiling 
					  the program
	$ dsedb -r run http_redirect.elf <URL> : Executes the program

	This program establishes an HTTP connection with a specified URL.  
	If redirection is specified by the server, the program establishes 
	a reconnection with the HTTP server where data is redirected.
...
< Operation Overview >
	...
	3. Establishes an HTTP connection again with the HTTP server 
	   where data is redirected and receives an HTTP response, if 
	   redirection code has been specified to the HTTP response.
