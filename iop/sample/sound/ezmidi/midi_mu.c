...
static char msinBf_T [MSIN_TRANS_SIZE];
static char msinBf_R [MSIN_TRANS_SIZE];
...
atick (void)
{
	...
	if (((MSInBuf *)msinBf_T)->flag != 0) { /* flag は msinBf_T の末端 */
	    memcpy (msinBf_R, msinBf_T, MSIN_TRANS_SIZE);
	    ((MSInBuf *)msinBf_T)->flag = 0;
	}
...
MidiTransBdPacket (EZMIDI_BANK *bank)
{
	...
	if (sceHSyn_VoiceTrans (1, (unsigned char *)gSpuTransPacketAddr,
