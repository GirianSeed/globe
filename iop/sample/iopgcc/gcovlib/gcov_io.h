...
/* Modify by SCEI for IOP
   "PlayStation 2" Programmer Tool Runtime Library Release 3.0
   $Id: gcov_io.h,v 1.1 2003/02/04 06:55:00 tei Exp $
 */
...
static int
__store_long (long value, char *dest, size_t bytes)
{
...
static int
__fetch_long ( long *dest, char *source, size_t bytes )
{
    long value = 0;
    int i;
    ...
    for (; i >= 0; i--)
	value = value * 256 + (source[i] & ((size_t)i == (bytes - 1) ?
					    127 : 255));
...
static int
__write_long (long value, int file, size_t bytes )
{
    char c[10];
	...
	return write(file, c, bytes) != bytes;
}
...
static int
__read_long (long *dest, int file, size_t bytes)
{
    char c[10];

    if (bytes > 10 || read(file, c, bytes) != bytes)
	return 1;
    else
