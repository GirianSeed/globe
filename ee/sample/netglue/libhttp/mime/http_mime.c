...
void subparts(sceHTTPMimeFilter_t *p){
	int closed;
	...
	while (1){
		...
		if (sceHTTPMimeFilterGetMultipartType(p)
			...
			if (sceHTTPFreeHeaderList(p->headers) < 0){
				printf("sceHTTPFreeHeaderList failed\n");
				ERR_STOP;
			}
			p->headers = NULL;
		}
...
int main(int argc, char **argv){
	...
	if(argc < 2){
		printf("usage: http_mime.elf url\n");
		ERR_STOP;
	}
	...
	if (sceHTTPMimeFilterGetMultipartType(fil))
		...
		if (sceHTTPMimeFilterApply(fil, NULL) < 0){
			printf("sceHTTPMimeFilterApply failed\n");
			ERR_STOP;
		}
	}

	if (sceHTTPMimeFilterGetStringOutput(fil, &odata, &olen) < 0){
		printf("sceHTTPMimeFilterGetStringOutput failed\n");
		ERR_STOP;
	}
