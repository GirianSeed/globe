 ...
 *       1.1.1        2002/12/04  ksh         DeleteThread,Sema & free added
...
PRIVATE	int recvmsg(sceSifMClientData *cd, u_int *buf, int cid, u_char *ptr, int count, int *pflags, int ms)
{
	int len, left = count;
...
PRIVATE	void make_connection(sceSifMClientData *cd, u_int *net_buf, int fid)
{
	...
	u_char trans_buf[BUFSIZE], mem[BUFSIZE];
	int ps;
	int flag;
	int i;
...
int main(int argc, char **argv)
{
	...
	***************************************************************/
	...
	for(i = 0; i < th_num; i++){
		...
		connection[i].tid = start_thread(thread_func, connection[i].stack, STACKSIZE, PRIO, &connection[i].fid);
	}

	for(i = 0; i < th_num; i++) {
		WaitSema(connection[i].sema);
		TerminateThread(connection[i].tid);
		DeleteThread(connection[i].tid);
