...
//0.5 ピクセルづつずらして描画した画像を４枚ブレンドすることにより
//アンチエイリアシングを実現します。画像のジッタリングと呼ぶことも
//あります。
//４回の描画を順々に行いますので、Ｚバッファが１画面で済みます。
...
#define LIGHT_RADIUS 2.0f
		...
		//NTSC なので、Y方向の解像度は 224 です。このため画素のずらし量は半分になります。
		//同じパケットをGSに４回送ります。ずらし量は XYOFFSET レジスターの変更で設定します。
		//set offset y+0.25
