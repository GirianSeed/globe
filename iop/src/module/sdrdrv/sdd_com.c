...
sce_sdr_loop (void)
{
    ...
    sceSifRegisterRpc (&rpc_sd, sce_SDR_DEV, sdrFunc, (void *)gRpcArg, NULL, NULL, &rpc_qd);
...
{ 
	...
	param.option       = 0;
	thid_cb = CreateThread (&param);
	StartThread (thid_cb, 0);
	Kprintf ("SDR callback thread created\n");
	break;

    case 0xe630:
	/* sceSdRemoteCallbackQuit() から呼ばれ、callback 用の
	 * スレッド (sce_sdrcb_loop) を terminate/delete する。
	 * RPC 用のスレッド (sce_sdr_loop) は module unload 時に
	 * 削除される。*/
	if (thid_cb > 0) {
