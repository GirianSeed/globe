 ...
 *      1.10            Nov, 6,2000    kumagae     Add InitGS
 */
#include <stdio.h>
#include <stdlib.h>
 ...
 * 日付,時刻を指定形式の文字列にする関数
 * char *str        文字列へのアドレス
 * sceCdCLOCK *prtc 年月日および時刻格納構造体アドレス
 * int datanotation 指定する日付形式
 * int timenotation 指定する時刻形式
 */
...
int ConsoleDraw(int console)
{
  ...
  sceDevConsMessage(console, 2,1, "PlayStation 2 System Configuration Sample Program Ver1.1");
...
void InitGS(void)
{
  sceVif1Packet packet;	/* DMAパケットハンドラ */
