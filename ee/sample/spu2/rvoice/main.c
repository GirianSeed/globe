...
#define DMA_CB_TEST 1
#define IRQ_CB_TEST 1
#define SIF_CB_TEST 1
...
int dataset(void)
{
	...
	vagAddr = (void*)sceSifAllocIopHeap(ALLOC_IOP_HEAP_SIZE);
	if( vagAddr < 0 ) { printf( "\nCan't alloc heap \n"); return -1; }
	PRINTF(("alloced 0x%x  ", vagAddr));
	i = sceSifLoadIopHeap(VAG_FILENAME, vagAddr);
