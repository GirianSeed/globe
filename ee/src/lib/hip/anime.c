 ...
 *                        All Rights Reserved.
 */
/*	$Id: anime.c,v 1.16 2003/12/12 06:46:29 xkazama Exp $	*/
#include <eekernel.h>
#include <stdio.h>
#include <eeregs.h>
#include <libvu0.h>
...
static sceHiErr anime_set(Anime_t *, u_int);
static int keypoint(u_int, u_int *, int);
static float spline(float, sceVu0FMATRIX, sceVu0FVECTOR);
static void Hermite(sceHiPlugHrchyData_t *, float *, int, float);
static void Bezier(sceHiPlugHrchyData_t *, float *, int, float);
static float linear(float, float, float);
static void Linear(sceHiPlugHrchyData_t *, float *, int, float);
static void Constant(sceHiPlugHrchyData_t *, float *, int);
...
static sceHiErr anime_init(sceHiPlug *p, Anime_t *anime)
{
    ...
    anime->frame_tbl = (u_int **)sceHiMemAlloc(sizeof(int *) * frame_head->key.num);
...
/*		Animation			*/
/************************************************/
static sceHiErr anime_set(Anime_t *anime, u_int frame)
{
...
/*		Hermite				*/
/************************************************/
static void Hermite(sceHiPlugHrchyData_t *hrc, float *kv, int key, float t)
{
  float f;
  u_int i;
  sceVu0FVECTOR *p;
...
/*		Bezier				*/
/************************************************/
static void Bezier(sceHiPlugHrchyData_t *hrc, float *kv, int key, float t)
{
  float f;
  u_int i;
  sceVu0FVECTOR *p;
...
static void Linear(sceHiPlugHrchyData_t *hrc, float *kv, int key, float t)
{
  float a,b;
  float f;
  u_int i;
...
/*		Constant			*/
/************************************************/
static void Constant(sceHiPlugHrchyData_t *hrc, float *kv, int key)
{
  sceVu0FVECTOR *p;
  u_int	*fcurve;
  u_int offset;
