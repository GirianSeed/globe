...
int main(int argc, char* argv[]){
	...
	*T0_COUNT = 0;		/* Hcounter reset */
	...
	sceRead(fd, (char *)&pad, sizeof(pad)); /* PAD read */

	if ((pad.button&SCE_PADLup) && nobj<MAXOBJ/SUNIT) {
	    nobj++;
	}else if ((pad.button&SCE_PADLdown) && nobj>1) {
	    nobj--;
	}
