...
Sample program to reflect a cookie in a connection to an HTTP server 
that uses a cookie

< Description of the Sample>
	This sample program reflects a cookie when connecting to an HTTP 
	server that uses a cookie, using the cookie feature of libhttp.
...
< Activating the Program >
	$ make                          : Links libhttp by compiling 
					  the program
	$ dsedb -r run http_cookie.elf <URL> : Executes the program

	This program establishes an HTTP connection with a specified URL.  
	If a cookie is specified by the server, the program reflects the 
	cookie in an HTTP request and establishes a connection again. 
...
< Operation Overview >
	...
	3. Reflects a cookie in an HTTP request, establishes an HTTP 
	   connection again, and receives an HTTP response, if a cookie 
	   has been specified to the HTTP response.
