 ...
 * ---------------------------------------------------------------- */
// 出力ポート
#define OUT_PORT_0   0		// 全ての MIDI ストリームはここに出力
...
/* CSL buffer context */
static sceCslBuffCtx msinBfCtx [OUT_PORT_NUM];// 出力ポートの総数

/*
 * MIDI stream buffer
 */
#define STREAMBUF_SIZE 1024	// バッファサイズ
#define STREAMBUF_SIZE_ALL (sizeof (sceCslMidiStream) + STREAMBUF_SIZE) // 属性 + バッファ
...
set_module_context (void)
{
    ...
    msinCtx.buffGrp    = msinGrp;
      ...
      msinGrp [OUT_BUF].buffCtx = msinBfCtx;
        // 出力ポート 0
	msinBfCtx [OUT_PORT_0].sema = 0;
	msinBfCtx [OUT_PORT_0].buff = &(streamBf [OUT_PORT_0]); // modhsyn と共有
...
set_msin (void)
{
    ...
    sceHSyn_SetVolume(&msinCtx, OUT_PORT_0, 0x7f);
    return 0;
}
...
atick (void)
{
	...
	sceMSIn_ProgramChange(&msinCtx, OUT_PORT_0, 0, 0);
	switch (count) {
	case 100:
	    printf ("Note on --- Comedy.\n");
	    //           module_Ctx addr, port, channel, note NO., vel
	    sceMSIn_NoteOn (&msinCtx, OUT_PORT_0, 0, 48, 0x7f); // ver. 1
	    break;

	case 400:
	    printf ("Note on --- Cheers!\n");
	    //           module_Ctx addr, port, channel, note NO., vel
	    sceMSIn_NoteOn (&msinCtx, OUT_PORT_0, 0, 72, 0x7f); // ver. 1
	    break;
	    
	case 800:
	  printf("Note on Ex --- telephone!\n");
	  // module_Ctx addr, port, channel(現在空いているもの), note, vel, program NO
	  sceMSIn_NoteOnEx(&msinCtx, OUT_PORT_0, 0, 96, 0x70, 0);
	  break;

	case 1800:
	    printf ("Note off.(Comedy, Cheers, telephone)\n");
	    //               Ctx addr, port, channel, note
	    sceMSIn_NoteOff (&msinCtx, OUT_PORT_0, 0, 48);
	    sceMSIn_NoteOff (&msinCtx, OUT_PORT_0, 0, 72);
	    sceMSIn_NoteOff (&msinCtx, OUT_PORT_0, 0, 96);
	    break;
	}
