...
AdpcmOpen (int ch, char *filename)
{
    ...
    if (gFd [ch] != NO_OPEN) {
	ERROR(("file is already opened. %s \n", filename));
	return -1;
    }
...
AdpcmClose (void)
{
    if (gFd [_L] >= 0) { close (gFd [_L]); gFd [_L] = NO_OPEN; }
    if (gFd [_R] >= 0) { close (gFd [_R]); gFd [_R] = NO_OPEN; }
    return;
}
...
sce_adpcm_loop (void)
{
    ...
    // 変数初期化
    gBufSpu [_L] = gBufSpu [_R] = NO_ASSIGN;
