...
#define INDEXIMG_ADDRESS_8_REF   6720
#define CLUT_ADDRESS_8     8000
...
u_long128 tex_buffer[256 * 256 / 16];
...
void load_tex_to_mem() {
    ...
    int fd;
#ifdef __32BIT__
    char filename[] = "host:tex8_32.raw";
#else
    char filename[] = "host:tex8.raw";
#endif

    fd = sceOpen(filename, SCE_RDONLY);
    sceRead(fd, (void*)tex_buffer, 256 * 256);
    sceClose(fd);
    return;
}
...
void load_clut_to_fb() {
        ...
        int fd;
        u_long128 buffer[256 * 4 / 16];

        fd = sceOpen("host:clut8.raw", SCE_RDONLY);
        sceRead(fd, (void*)buffer, 256 * 4);
...
void init_packet(Q_WORDDATA *pbuf) {
	...
	pbuf[4].ul64[0] = SCE_GS_SET_TEX0(INDEXIMG_ADDRESS_8_REF, 4, SCE_GS_PSMT8, 8, 8, 1, 1, CLUT_ADDRESS_8, SCE_GS_PSMCT32, 0, 0, 1);

	//pbuf[5].ul64[0] = SCE_GS_SET_TEX1(0, 0, SCE_GS_NEAREST, SCE_GS_NEAREST, 0, 0, 0);
...
#ifdef __32BIT__
	sceGsSetDefLoadImage(&tp, INDEXIMG_ADDRESS_8, 2, SCE_GS_PSMCT32, 0, 0, 128, 128);
#else
