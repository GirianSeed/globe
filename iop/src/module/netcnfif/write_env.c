 ...
 *       1.3       2002.07.04      tetsu       Correspond to CodeWarrior
 *       1.4       2002.10.19      tetsu       Change path of netcnfif.h
 *       1.5       2003.09.24      jwang       !STATIC_NETCNFIF_DATA
 */
...
/*
 *  dup_string allocates storage from the sceNetCnfEnv work area,
 *  allowing "deep-copy" of sceNetcnfifData to sceNetcnfEnv.
 */
#if (!STATIC_NETCNFIF_DATA)
static u_char *dup_string(sceNetCnfEnv_t *e, u_char *str)
{
	u_char *r;

	if(NULL == (r = sceNetCnfAllocMem(e, strlen(str) + 1, 0)))
		return(NULL);
	strcpy(r, str);
	return(r);
}
#endif  // STATIC_NETCNFIF_DATA
...
MAKE_PAIR:
	...
	/* リンクを作成 */
	if((p->back = e->root->pair_tail) != NULL){
	    p->back->forw = p;
	}else{
	    e->root->pair_head = p;
	}
...
#else
	    e->ifc->dhcp_host_name = dup_string(e, data->dhcp_host_name);
#endif  // !STATIC_NETCNFIF_DATA
	    init_flag = 0;
	}
...
#else
	    e->ifc->address = dup_string(e, data->address);
#endif  // !STATIC_NETCNFIF_DATA
	    init_flag = 0;
	}
...
#else
	    e->ifc->netmask = dup_string(e, data->netmask);
#endif  // !STATIC_NETCNFIF_DATA
	    init_flag = 0;
	}
...
#else
	    e->ifc->phone_numbers[0] = dup_string(e, data->phone_numbers1);
#endif  // !STATIC_NETCNFIF_DATA
	    init_flag = 0;
	}
...
#else
	    e->ifc->phone_numbers[1] = dup_string(e, data->phone_numbers2);
#endif  // !STATIC_NETCNFIF_DATA
	    init_flag = 0;
	}
...
#else
	    e->ifc->phone_numbers[2] = dup_string(e, data->phone_numbers3);
#endif  // !STATIC_NETCNFIF_DATA
	    init_flag = 0;
	}
...
#else
	    e->ifc->auth_name = dup_string(e, data->auth_name);
#endif  // !STATIC_NETCNFIF_DATA
	    init_flag = 0;
	}
...
#else
	    e->ifc->auth_key = dup_string(e, data->auth_key);
#endif  // !STATIC_NETCNFIF_DATA
	    init_flag = 0;
	}
...
#else
	    e->ifc->peer_name = dup_string(e, data->peer_name);
#endif  // !STATIC_NETCNFIF_DATA
	    init_flag = 0;
	}
...
#else
	    e->ifc->vendor = dup_string(e, data->vendor);
#endif  // !STATIC_NETCNFIF_DATA
	    init_flag = 0;
	}
...
#else
	    e->ifc->product = dup_string(e, data->product);
#endif  // !STATIC_NETCNFIF_DATA
	    init_flag = 0;
	}
...
#else
	    e->ifc->chat_additional = dup_string(e, chat_additional);
#endif  // !STATIC_NETCNFIF_DATA
	    init_flag = 0;
	}
...
#else
	    e->ifc->outside_number = dup_string(e, data->outside_number);
#endif  // !STATIC_NETCNFIF_DATA
	    init_flag = 0;
	}
...
#else
	    e->ifc->outside_delay = dup_string(e, data->outside_delay);
#endif  // !STATIC_NETCNFIF_DATA
	    init_flag = 0;
	}
...
#if (!STATIC_NETCNFIF_DATA)
    if (e->alloc_err) return (sceNETCNFIF_ALLOC_ERROR);
#endif  // !STATIC_NETCNFIF_DATA
...
#else
    p->display_name = dup_string(e, display_name);
#endif  // !STATIC_NETCNFIF_DATA
...
#else
    p->attach_ifc = dup_string(e, data->attach_ifc);
#endif  // !STATIC_NETCNFIF_DATA
...
#else
    p->attach_dev = dup_string(e, data->attach_dev);
#endif  // !STATIC_NETCNFIF_DATA
...
int sceNetcnfifWriteEnv(sceNetCnfEnv_t *e, sceNetcnfifData_t *data, int type)
{
    ...
    printf("%s: %d used out of %d, %d remaining\n",
            __FUNCTION__,
            (u_int)(e->mem_ptr  - e->mem_base),
            (u_int)(e->mem_last - e->mem_base),
            (u_int)(e->mem_last - e->mem_ptr) );
#endif
