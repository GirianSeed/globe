	...
	for(j=i+1;j<nbody;j++) {
	  ...
	  if (cl[count].bodyA->type!=PASSIVE || cl[count].bodyB->type!=PASSIVE) {
	    ...
	    if (cl[count].dist<COLL_DIST) {
	      // seperate objects if they intersect
	      if(cl[count].dist<SEPARATE_DIST) {
		time=get_coll_time(tstep,tstep,&(cl[count]),Xstart,Xend,mem->d1,mem->d5,i,j,SEPARATE_DIST,COLL_DIST);
                if (time<coll_time) {
                      coll_time=t;index1=i;index2=j;             
                }
	      }	  	            
	    }
	  }	
      ...
      if (coll_time<tstep) 
         roll_back(coll_time,tstep,Xstart,Xend,mem->d1,mem->d5,index1,index2,bodylist,nbody);  
      
      if (coll_time==0.0f) printf("could not find earliest collision!!!\n");

    // add impulse checking all collision pairs is only needed if 2 or more collisions happen at the same time
    count=-1;
    for(i=0;i<nbody-1;i++) {
      for(j=i+1;j<nbody;j++) { 
       count++; 
