...
// max length of data part
#define MAX_DBG_MEM_LEN     (DECI2_MAX_LEN - sizeof(DECI2_HDR) \
		- sizeof(DBGP_HDR) - sizeof(DBGP_MEM))

static DBGP_CONF_DATA cf;
static MaxMemAlign = DBGP_ALIGN_BYTE;		// max align value for WRMEM packet

// make, send DBGP RUN packet, and wait replay
...
// make, send DBGP WRMEM packet, and wait replay
static int DbgpWrMem(u_char align, u_int adr, void *ptr, int len) {
	...
	while (len > 0) {
		...
		if (cf.v1.mem_limit_align & (1 << align))
			if (nhdr + npad + n > cf.v1.mem_limit_size) {
...
// down load data to adr
// strategy for fast download:
// 1, send leading unaligned fraction by DBGP_ALIGN_BYTE
// 2, send aligned data by MaxMemAlign(specified in getconf packet)
// 3, send rest of unaligned data by DBGP_ALIGN_BYTE
static int DbgpDownLoad(int adr, void *ptr, int len) {
	int asize, amask, off, n;

	amask = (asize = (1 << MaxMemAlign)) - 1;

	if ((off = (adr & amask)) > 0) {		// send leading unaligned fraction
	...
	if ((n = (len & ~amask)) > 0) {			// send aligned data
		if (DbgpWrMem(MaxMemAlign, adr, ptr, n) < 0) return -1;
		adr += n;
		ptr += n;
		len -= n;
	}
	if ((n = len) > 0) {					// send rest of unaligned fraction
...
static void *DbgpMakeGetconf(void) {			// make getconf packet
	struct _pkt {
		DECI2_HDR deci2;
		DBGP_HDR dbgp;
		DBGP_CONF_DATA conf;
...
int DbgpPload(char *filename) {
	...
	// make DBGP getconf packet, send it and wait reply
	...
	for (i=0; i<ehdr.e_shnum; i++) {		// find reginfo section
		if (shdrTbl[i].sh_type == SHT_MIPS_REGINFO) {
			...
			gp = ri->ri_gp_value;			// get GP value
//			printf("gp_value = 0x%08x\n", gp);
			free(ri);
			break;
		}
	}

	// make DBGP run packet, send it and wait reply
