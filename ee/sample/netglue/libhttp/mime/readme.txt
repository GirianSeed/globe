...
Sample program that uses libhttp to connect to an HTTP server, then parses the MIME multipart from the response.

<Description>
This is a sample program that uses libhttp to connect to an HTTP server then parses the MIME multipart from the response.
...
<Run Method>
	...
	$ dsedb -r run http_mime.elf <URL>	: Execute

An HTTP connection is made to the specified URL, the MIME multipart is parsed from the HTTP response, and the first part of the parsed result is displayed.
...
3. Create a MIME multipart filter and recursively perform filtering on the HTTP response.
4. Display the first part of the analyzed result.
