...
libhttp を利用して HTTP サーバに接続するサンプル。

<サンプルの解説>
	本プログラムは libhttp を利用して HTTP サーバに
	接続するサンプルです。
	libhttp は HTTP 接続の際にブロッキング、ノンブロッキングを
	選択できますが、本サンプルはノンブロッキングで接続するものです。
...
<ファイル>
	http_test.c        : メインプログラム

<起動方法>
	$ make                          : コンパイル

	$ dsedb -r run http_test.elf command <URL> [post-data/file] : 実行

	指定された URL に対して HTTP 接続を行ない、HTTP レスポンスを
	表示します。

	command は HTTP メソッドを指定するもので、以下の3つを指定できます。
		-get	GET メソッド
		-head	HEAD メソッド
		-post	POST メソッド
	POST メソッドの場合は、post-data として POST するデータを
	指定できます。また GET メソッドの場合は応答のボディ部分
	を受信するファイル名を指定することができます。
...
<処理概略>
	...
	2. sceHTTPSetOption() でノンブロッキングを指定する
	3. 目的の URL に対して HTTP 接続し、HTTP レスポンスを取得する
	4. HTTP レスポンスを表示する
