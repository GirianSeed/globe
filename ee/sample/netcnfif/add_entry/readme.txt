...
This sample program uses the common network configuration interface library to add a Your Network Configuration file.
...
1. Insert a PS2 memory card that does not contain any Your Network Configuration files into memory card slot 1.
...
3. Execute make in the directory /usr/local/sce/ee/sample/netcnfif/add_entry,

	   > cd /usr/local/sce/ee/sample/netcnfif/add_entry
	   > make
...
6. The add will have completed when the following information is displayed.

	   [add_entry.c] complete
...
1. Confirm that sceNetcnfifSetFNoDecode(sceNetcnfifArg_f_no_decode_on); is executed in /usr/local/sce/ee/sample/netcnfif/add_entry/add_entry.c. (If this is not the case, the contents of the Your Network Configuration file cannot be verified because they will be encoded.)
...
6. Confirm the contents on the dsecons screen by moving to the device where the Your Network Configuration file was saved.
	   ...
	   d------rwx 0     0     0x0027     0         9 Jan 24  5:36 BWNETCNF
	   [mc0:]/ $ cd BWNETCNF
	   [mc0:]/BWNETCNF $ ls
	   ...
	   -------rwx 0     0     0x0017     0       113 Jan 24  5:36 dev000.cnf
	   -------rwx 0     0     0x0017     0       110 Jan 24  5:36 BWNETCNF
	   -------rwx 0     0     0x0017     0       207 Jan 24  5:36 ifc000.cnf
	   -------rwx 0     0     0x0017     0       100 Jan 24  5:36 net000.cnf
	   [mc0:]/BWNETCNF $ 

7. Confirm that the directories and files shown above have been created.

8. Confirm the contents of the configuration management file (BWNETCNF).

The contents depend on the following definitions, which are set in /usr/local/sce/ee/sample/netcnfif/add_entry/add_entry.c.

	   #define NET_USR_NAME "Combination1"
	   #define IFC_USR_NAME "SCE Ethernet Setting"

	   [mc0:]/BWNETCNF $ cat BWNETCNF
	   0,1,net000.cnf,Combination1
	   1,1,ifc000.cnf,SCE Ethernet Setting
	   2,1,dev000.cnf,SCE/Ethernet (Network Adaptor)

	   [mc0:]/BWNETCNF $ 

9. Confirm the contents of the combination (net000.cnf).

	   [mc0:]/BWNETCNF $ cat net000.cnf
	   # <Sony Computer Entertainment Inc.>

	   interface "ifc000.cnf + dev000.cnf" "ifc000.cnf" "dev000.cnf"

	   [mc0:]/BWNETCNF $ 

10. Confirm the contents of the hardware settings (dev000.cnf) and Internet service provider settings (ifc000.cnf).
