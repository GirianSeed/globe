...
libhttp を利用して、
HTTP 認証(BASIC認証、ダイジェスト認証) を必要とする HTTP サーバに
接続するサンプル。

<サンプルの解説>
	本プログラムは libhttp の HTTP 認証機能を利用して、
	認証を求める HTTP サーバに接続するサンプルです。
	libhttp はBASIC 認証とダイジェスト認証に対応して
	おり、本サンプルでもどちらか要求された認証を行ないます。
...
<ファイル>
	http_auth.c        : メインプログラム

<起動方法>
	$ make                          : コンパイル

	$ dsedb -r run http_auth.elf <URL> <user> <passwd> : 実行

	指定された URL に対して HTTP 接続を行ない、認証が必要である旨の
	HTTP レスポンスを受信した場合には、ユーザ名 user、パスワード 
	passwd を基に HTTP リクエストを作成し、再接続を行ないます。
...
<処理概略>
	...
	3. HTTP レスポンスとして「認証されてない(401)」が返ってき
	   ていれば、ユーザ名 user、パスワード passwd を基に認証データを
	   作成し、再度 HTTP 接続し、HTTPレスポンスを取得する
	4. 結果を表示する
