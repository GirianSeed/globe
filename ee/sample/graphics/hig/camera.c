...
sceVu0FMATRIX sm = {{512, 0, 0, 0}, {0, 240.639999, 0, 0}, {2048, 2048, -251.000534, 1}, {0, 0, 16777250, 0}};	/* screen matrix */
...
/*
  ...
  NOTE:
  * screen Height-size must be the Height-size of the  frame buffer
  * (not off-screen height size)
*/
...
void cameraview(sceHiPlugMicroData_t *md)
{
    ...
    sceVu0CopyVectorXYZ(&md->camx, xd);
}

/*	dst <- src	*/
void cameracopy(sceHiPlugMicroData_t *dst, sceHiPlugMicroData_t *src)
{
    sceVu0CopyMatrix(dst->wscreen, src->wscreen);
    sceVu0CopyMatrix(dst->mtx.wview, src->mtx.wview);
    sceVu0CopyVectorXYZ(&dst->camx, &src->camx);
}
...
void pilotview(sceHiPlugMicroData_t *md)
{
    ...
    sceVu0UnitMatrix(m);
    sceVu0ScaleVector(pos, position, -1.0f);
    ...
    /*	for specific micro code	*/
    sceVu0CopyMatrix(md->mtx.wview, m);
    sceVu0CopyVectorXYZ(&md->camx, position);
}

/************************************************/
/*	camera data control			*/
/************************************************/
void cameradatactrl(sceHiPlugCameraData_t *cd)
{
    if(gPad.RrightOn){	cd->position[X] += 1.0f; cd->interest[X] += 1.0f; }
    if(gPad.RleftOn){	cd->position[X] -= 1.0f; cd->interest[X] -= 1.0f; }
    if(gPad.RdownOn){	cd->position[Y] += 1.0f; cd->interest[Y] += 1.0f; }
    if(gPad.RupOn){	cd->position[Y] -= 1.0f; cd->interest[Y] -= 1.0f; }
    if(gPad.R1On){	cd->position[Z] += 1.0f; cd->interest[Z] += 1.0f; }
    if(gPad.R2On){	cd->position[Z] -= 1.0f; cd->interest[Z] -= 1.0f; }

    if(gPad.LupOn){	cd->rotation[X] -= 0.1f; if(cd->rotation[X]<-PI) cd->rotation[X]+=2.0f*PI; }
    if(gPad.LdownOn){	cd->rotation[X] += 0.1f; if(cd->rotation[X]>PI)  cd->rotation[X]-=2.0f*PI; }
    if(gPad.LleftOn){	cd->rotation[Y] -= 0.1f; if(cd->rotation[Y]<-PI) cd->rotation[Y]+=2.0f*PI; }
    if(gPad.LrightOn){	cd->rotation[Y] += 0.1f; if(cd->rotation[Y]>PI)  cd->rotation[Y]-=2.0f*PI; }
    if(gPad.L2On){	cd->rotation[Z] -= 0.1f; if(cd->rotation[Z]<-PI) cd->rotation[Z]+=2.0f*PI; }
    if(gPad.L1On){	cd->rotation[Z] += 0.1f; if(cd->rotation[Z]>PI)  cd->rotation[Z]-=2.0f*PI; }

    draw_debug_print(1, 10, "  POS  %5.2f %5.2f %5.2f", cd->position[X],cd->position[Y],cd->position[Z]);
    draw_debug_print(1, 11, "  ROT  %5.2f %5.2f %5.2f", cd->rotation[X],cd->rotation[Y],cd->rotation[Z]);
}
