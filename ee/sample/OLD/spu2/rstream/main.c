...
void dataset(void)
{
	...
	while (sceSifLoadIopHeap("host0:/usr/local/sce/data/sound/wave/tr1r_pad.vb", (void*)data_start_addr[0]) < 0) {
	    printf( "\nCan't load R-ch file to iop heap \n"); 
	}
	while (sceSifLoadIopHeap("host0:/usr/local/sce/data/sound/wave/tr1l_pad.vb", (void*)data_start_addr[1] ) < 0) {
