 ...
 *                        All Rights Reserved.
 */
/*	$Id: tim2.c,v 1.7 2001/10/29 05:29:55 kaneko Exp $	*/
#include <eekernel.h>
#include <stdlib.h>
#include <eeregs.h>
#include <libgraph.h>
...
static const char *_tim2_err_mes[] = {
    ...
    "HiP Tim2 : this TIM2 data is not allowed(version mismatch)\n",
    "HiP Tim2 : the argument is too huge for this data\n",
    ...
    "HiP Tim2 : the plugin's stack is broken(maybe didnot do INIT_PROCESS)\n",
};
...
static sceHiErr GetTim2PictureHeader(u_int *, int, TIM2_PICTUREHEADER **);
static void MakeTim2TexturePacket(int, u_int, int, int, int, int, int, u_long128 *);
...
static sceHiErr DecodeTim2(TIM2_FRAME *, u_int *, u_int *, sceHiGsMemTbl *);
#if __DEBUG
...
*/
static sceHiErr GetTim2PictureHeader(u_int *data, int num, TIM2_PICTUREHEADER **addr)
{
    TIM2_FILEHEADER *pFileHdr = (TIM2_FILEHEADER *)data;
    TIM2_PICTUREHEADER *pPictHdr;
    sceHiErr err;
...
*/
static void MakeTim2TexturePacket(int psm, u_int tbp, int tbw, int w, int h, int x, int y, u_long128 *pImage)
{
    int	n;
    qword	tag[10];
    int	size;
    ...
    sceHiDMAMake_LoadGSLump((u_int *)&(tag[0]), 5);
    sceHiDMAMake_LoadGSLump((u_int *)&(tag[5]), 1);
    sceHiDMAGet_BufferPtr(&(_curtf->pbufaddr[_curimg]));
    sceHiDMAMake_LoadGS((u_int *)pImage, size);
    sceHiDMAMake_LoadGSLump((u_int *)&(tag[6]), 2);
}
...
static sceHiErr DecodeTim2Image(TIM2_PICTUREHEADER *ph, sceHiGsGiftag *env)
{
    ...
    MakeTim2TexturePacket(psm, tbp, tbw, w, h, 0, 0, pImage);
...
static sceHiErr DecodeTim2Clut(TIM2_PICTUREHEADER *ph, sceHiGsGiftag *env)
{
	...
	MakeTim2TexturePacket(cpsm, cbp, 1, 16, (ph->ClutColors / 16), 0, 0, pClut);
	break;
	...
	for (i = 0; i < (ph->ClutColors / 16); i++) {
	    ...
	    MakeTim2TexturePacket(cpsm, cbp, 1, 8, 2, (i & 1) * 8, (i >> 1) * 2, pClut);
	    if ((ph->ClutType & 0x3f) == TIM2_RGB16) {
...
/*
  ...
  tbl : GS Memory Table
*/
static sceHiErr DecodeTim2(TIM2_FRAME *tim2, u_int *data, u_int *env, sceHiGsMemTbl *tbl)
{ 
    typedef struct {
	u_int	reserve[3];
	u_int	num;
    ...
    _tim2_data_t	*tim2dat;
    _tim2_item_t	*tim2itm;
    sceHiGsGiftag	*tags;
    ...
    tim2dat = (_tim2_data_t *) data;
    tim2itm = (_tim2_item_t *)(tim2dat + 1);
#if __DEBUG
    for (i = 0; i < tim2dat->num; i++) {
	printf("---- %d ----\n", i);
	TIM2INFO(tim2itm[i].dataptr);
    }
#endif
    /* packet making */
    sceHiDMAMake_ChainStart();
    for (i = 0; i < tim2dat->num; i ++) {
	if ((err = GetTim2PictureHeader(tim2itm[i].dataptr, 0, &ph)) != SCE_HIG_NO_ERR)
	    return err;
	tags = getPackedGiftag((u_int *)env, tim2itm[i].tim2id);
	if ((err = DecodeTim2Image(ph, tags)) != SCE_HIG_NO_ERR)
	    return err;
	if ((err = DecodeTim2Clut(ph, tags)) != SCE_HIG_NO_ERR)
	    return err;	
    }
    sceHiDMAMake_ChainEnd(&(tim2->id));
...
sceHiErr sceHiPlugTim2(sceHiPlug *plug, int process)
{
	...
	err = DecodeTim2(tim2_frame, (u_int *)data, (u_int *)env, tbl);
	if (err != SCE_HIG_NO_ERR)
	    return err;
	...
	if (tim2_frame == NULL)
	    return _hip_tim2_err(_PLGBLK_STACK_BROKEN);
	sceHiDMADel_Chain(tim2_frame->id);
...
int sceHiPlugTim2Num(sceHiPlug *plug)
{
    ...
    type.id		= SCE_HIP_TIM2_DATA;
    err = sceHiGetData(plug, &data, type);
    if (err != SCE_HIG_NO_ERR)	return -1;
    return data[3];
}
...
char *sceHiPlugTim2GetName(sceHiPlug *plug, int idx)
{
    ...
    type.id		= SCE_HIP_TIM2_DATA;
    err = sceHiGetData(plug, &data, type);
    if (err != SCE_HIG_NO_ERR)	return NULL;
    if (data[3] <= idx)	return NULL;
    data += 4;
...
sceHiErr sceHiPlugTim2GetNPictures(sceHiPlug *plug, int n, int *num)
{
    ...
    tim2dat = (_tim2_data_t *)data;
    tim2itm = (_tim2_item_t *)(tim2dat + 1);
    pfh = (TIM2_FILEHEADER *)tim2itm[n].dataptr;
    *num = pfh->Pictures;
    return SCE_HIG_NO_ERR;
}
...
sceHiErr sceHiPlugTim2SetPicture(sceHiPlug *plug, int n, int num)
{
    ...
    sceHiDMAGet_BufferPtr(&save); {
	/* picture */
	GetTim2PictureHeader(tim2itm[n].dataptr, num, &ph);
	pm = (TIM2_MIPMAPHEADER *)(ph + 1);
	pImage = (u_long128 *)((char *)ph + ph->HeaderSize);
	psm = psmtbl[ph->ImageType - 1];
	w = ph->ImageWidth;
...
#endif
	    sceHiDMASet_BufferPtr(tf->pbufaddr[0]);
	    sceHiDMAMake_LoadGS((u_int *)pImage, _calc_image_size(psm, w, h));
	}
	for (i = 1; i < ph->MipMapTextures; i++) {
	    w = w / 2;
	    h = h / 2;
...
#endif
		sceHiDMASet_BufferPtr(tf->pbufaddr[i]);
		sceHiDMAMake_LoadGS((u_int *)pImage, _calc_image_size(psm, w, h));
	    }
	}
	...
	if (ph->ClutType != TIM2_NONE) {
	    ...
	    switch ((ph->ClutType << 8) | ph->ImageType) {
	      ...
	      case (( TIM2_RGB32         << 8) | TIM2_IDTEX8): /* 256 CSM1 32bit */
		{
		    sceHiDMASet_BufferPtr(tf->pbufaddr[7]);
		    sceHiDMAMake_LoadGS((u_int *)pClut, _calc_image_size(psm, w, h));
		}
		break;
	      case (( TIM2_RGB16         << 8) | TIM2_IDTEX4): /* 16 CSM1 16bit */
	      case (( TIM2_RGB24         << 8) | TIM2_IDTEX4): /* 16 CSM1 24bit */
