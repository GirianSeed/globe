 ...
 *                        All Rights Reserved.
 */
/*	$Id: tim2.c,v 1.7.8.1 2002/02/22 15:09:30 aoki Exp $	*/
#include <eekernel.h>
#include <stdlib.h>
#include <eeregs.h>
#include <libgraph.h>
...
static const char *_tim2_err_mes[] = {
    ...
    "HiP Tim2 : the tim2 data address is NULL.(maybe it is not set yet)\n"
};
...
sceHiErr sceHiPlugTim2SetPicture(sceHiPlug *plug, int n, int num)
{
    ...
    {
	/* picture */
	err = GetTim2PictureHeader(tim2itm[n].dataptr, num, &ph);
	if (err != SCE_HIG_NO_ERR)
	    return err;
	pm = (TIM2_MIPMAPHEADER *)(ph + 1);
	pImage = (u_long128 *)((char *)ph + ph->HeaderSize);
